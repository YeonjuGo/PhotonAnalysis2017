...
void correlation_genPt_recoPt_v3_test(TString coll="pbpb", TString ver="190703_temp_v31_noEnergyCorrection", bool splitMC=false, bool isFineBinning=false, bool doWeight=true){
   ...
   // TMatrix mymatrix(nbinsx+2,nbinsy+2,myhisto.GetArray(),"F") 
    ...
    if(splitMC==0){
        ...
        for(Int_t i=0;i<nCENTBINS;++i){
            ...
            if(doWeight)    t1->Draw(Form("%s>>%s","phoEt",h1D_pt[2][i]->GetName()), Form("(%s)*((%s))","weight",commonCut.GetTitle()));
            else            t1->Draw(Form("%s>>%s","phoEt",h1D_pt[2][i]->GetName()), commonCut);
            ...
            //h2D fineBinning
            if(doWeight)    t1->Draw(Form("%s:%s>>%s","pho_genEt","phoEt",h2D_gen_reco[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1->Draw(Form("%s:%s>>%s","pho_genEt","phoEt",h2D_gen_reco[i]->GetName()), mcIsolation && commonCut);
        ...
        for(Int_t i=0;i<nCENTBINS;++i){
            ...
            if(doWeight)    t1_even->Draw(Form("%s>>%s","phoEt",h1D_pt[1][i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1_even->Draw(Form("%s>>%s","phoEt",h1D_pt[1][i]->GetName()), mcIsolation && commonCut);
            h1D_pt[1][i]=(TH1D*)gDirectory->Get(h1D_pt[1][i]->GetName());
            ...
            if(doWeight)    t1_odd->Draw(Form("%s:%s>>%s","pho_genEt","phoEt",h2D_gen_reco[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1_odd->Draw(Form("%s:%s>>%s","pho_genEt","phoEt",h2D_gen_reco[i]->GetName()), mcIsolation && commonCut);
            h2D_gen_reco[i]=(TH2D*)gDirectory->Get(h2D_gen_reco[i]->GetName());
            h1D_pt_ratio[i]->Divide(h1D_pt[1][i],h1D_pt[0][i]);
            //h1D_pt_ratio_error[i] = getRatioErrorHist_completelyCorrelated(h1D_pt_ratio[i],h1D_pt[1][i],h1D_pt[0][i]); 
        }
    ...
    TCanvas* c2;
   if(coll=="pp") c2 =new TCanvas(Form("c%d",2),"", 400+50,400*2);
