...
void compare_sumIso_sig_sideband(TString coll="pbpb", TString ver="190703_temp_v31", bool isBkg=true, bool isConstantSB = false, double sigCut = 0.010, int Nfunc = 0){
    ...
    const double ptBins_i_[] = {20,20,50,20,25,30,40,50,60,80,100,130};
    const double ptBins_f_[] = {200,50,200,25,30,40,50,60,80,100,130,200};
    int nPtBinIF_ = sizeof(ptBins_i_)/sizeof(double);
    
    //const double tempSig_i[] = {10};
    //const double tempSig_f[] = {100};
    const double tempSig_i[] = {0.011,0.012,0.011,0.010};
    const double tempSig_f[] = {0.015,0.015,0.013,0.014};
    //const double tempSig_i[] = {2,2.5,3,4,5,6,8,10,13,20};
    //const double tempSig_f[] = {7,7.5,8,9,10,11,13,15,18,25};
    //const double tempSig_i[] = {2,2.5,3,4,5,6,8,10,13,20,     2,2.5,3,4,5,6,8,10,13,20};
    //lconst double tempSig_f[] = {7,7.5,8,9,10,11,13,15,18,25, 12,12.5,13,14,15,16,18,20,23,30};
   
    //const double tempSig_i[] = {2,3,4,6,8,10,14,18,24,  2,3,4,6,8,10,14,18,24};
    //const double tempSig_f[] = {7,8,9,11,13,15,19,23,29,12,13,14,16,18,20,24,28,34};
    int nTempSig = sizeof(tempSig_i)/sizeof(double);
    if(isConstantSB==true) nTempSig = 1;
    ...
    for(Int_t i = 0; i < nPtBinIF_; ++i) 
    {
        ...
        for(Int_t k = 0; k < nCENTBINS; ++k){ 
            ...
            for(Int_t ss = 0; ss < nTempSig; ++ss){ 
                if(coll=="pp") {
                    //int tempSig_pp_i = 0.2*ptBins_i_[i]-2;
                    //int tempSig_pp_f = 0.2*ptBins_f_[i]-2+interval;
                    ...
                    //sigmaBkgCut = commonCut && isoCut_pp;
                    //nonsigmaBkgCut = commonCut && nonIsoSBCut_pp;
                    sigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 < %f", sigCut);
                    nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f", (double)tempSig_i[ss],(double)tempSig_f[ss]);
                    //nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f", (double)tempSig_pp_i,(double)tempSig_pp_f);
                    //if(isConstantSB) nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f", sideband_i,sideband_f);
                } else{//PbPb
                    if(isBkg) commonCut = mcBkgIsolation && hoeCut && ptCut_ && etaCut && centCut;
                    else commonCut = mcIsolation && hoeCut && ptCut_ && etaCut && centCut;
                    sigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 < %f", sigCut);
                    nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f", (double)tempSig_i[ss],(double)tempSig_f[ss]);
                    //nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f",temp_sumIso_pbpb_i[k] ,temp_sumIso_pbpb_i[k]+interval);
                    //if(isConstantSB) nonsigmaBkgCut = commonCut && Form("phoSigmaIEtaIEta_2012 > %f && phoSigmaIEtaIEta_2012 < %f",sideband_i ,sideband_f);
                }
                ...
                //     sigmaBkgCut = commonCut && isoCut_pp;
                //     nonsigmaBkgCut = commonCut &&  isoCut_pp && hoeCut_pp;
                // } else{//PbPb
                //     if(isBkg) commonCut = mcBkgIsolation && ptCut_ && etaCut && centCut;
                //     else commonCut = mcIsolation && ptCut_ && etaCut && centCut;
                //     sigmaBkgCut = commonCut && isoCut;
                //     nonsigmaBkgCut = commonCut && isoCut && hoeCut;
                // }
                ...
                TString cap2 = ""; 
                if(isBkg){ 
                    //cap2 = Form("bkgPhotons_sigSigma0p010_sidebandSigma%dto%d",(int)sideband_i,(int)sideband_f);
                    if(coll=="pp") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%.3fto%.3f",(float)tempSig_i[ss],(float)tempSig_f[ss]);
                    if(coll=="pbpb") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%.3fto%.3f",(float)tempSig_i[ss],(float)tempSig_f[ss]);
                    //if(isConstantSB) cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%.3fto%.3f",(float)sideband_i,(float)sideband_f);
                   // if(coll=="pp") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%dto%d",(int)tempSig_i[ss],(int)tempSig_f[ss]);
                   // if(coll=="pbpb") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%dto%d",(int)tempSig_i[ss],(int)tempSig_f[ss]);
                   // 
                    //if(coll=="pp") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%dto%d",(int)(0.2*ptBins_i_[i]-2),(int)(0.2*ptBins_i_[i]-2+interval));
                    //if(coll=="pbpb") cap2 = Form("bkgPhotons_sigSigma0.010_sidebandSigma%dto%d",(int)temp_sumIso_pbpb_i[k],(int)(temp_sumIso_pbpb_i[k]+interval));
                } else{ 
                    cout << "" << endl;
                    //cap2 = Form("sigPhotons_sigSigma0.010_sidebandSigma%dto%d",(int)sideband_i,(int)sideband_f);
                }
                //cap2+= "_fitRange0p008to0p015";
                //const char* cap2 = "mcTotWeighted_fitChi2_shift-0.0003";
                cout << "start" << endl;
                if(coll=="pp") compareTwo(t1, t1, "pho_sumIsoCorrected",150, -50, 100, sigmaBkgCut, nonsigmaBkgCut,cap,cap2,isBkg,Nfunc);
                else compareTwo(t1, t1, "pho_sumIsoCorrected",150, -50, 100, sigmaBkgCut, nonsigmaBkgCut ,cap,cap2,isBkg,Nfunc); 
            }
        }
    }
} // main function
...
void compareTwo(TTree* t1, TTree* t2, TString var, int nBins, double xMin, double xMax, TCut cut1, TCut cut2, TString cap, TString cap2, bool isBkg, int Nfunc)  {
   ...
   // fratio->Draw("same");
    ...
    TFile* fout = new TFile(Form("%soutput/sumIso_sig_sideband_%s_%s_function%d.root",dir.Data(),cap.Data(),cap2.Data(),Nfunc),"RECREATE");
    f1->Write();
    c->Write();
    h1->Write();
    h2->Write();
