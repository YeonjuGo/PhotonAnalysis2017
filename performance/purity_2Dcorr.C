...
void purity_2Dcorr(TString ver = "190703_temp_v31"){
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
    SetyjPadStyle();
    ...
    double ymax = 0.05;
    //double ymax = 0.3;
    TH2D* h2D = new TH2D("h2D",";photon E_{T} (GeV);sumIso (GeV)",180,20,200,300,-100,200);
    //TH2D* h2D = new TH2D("h2D",";sumIso;photon E_{T}",300,-100,200,180,20,200);
    //for MC Bkg
    t1->Draw("pho_sumIsoCorrected:phoEtCorrected>>h2D",Form("(1.)*(%s && %s && %s && %s && %s && %s)",mcBkgIsolation.GetTitle(),noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
    //for MC
    //t1->Draw("pho_sumIsoCorrected:phoEtCorrected>>h2D",Form("(1.)*(%s && %s && %s && %s && %s && %s)",mcIsolation.GetTitle(),noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
    //for DATA
    //t1->Draw("pho_sumIsoCorrected:phoEtCorrected>>h2D",Form("(1.)*(%s && %s && %s && %s && %s)",noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
   ...
   //fitting
    TF1* f_pol = new TF1("f_pol", "pol1",20,100);
