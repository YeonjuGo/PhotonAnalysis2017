...
void compare_AllQCD_EmEnr_ROC(TString coll="pbpb"){
    ...
    for(Int_t ipt = 1; ipt < nPtBinIF; ++ipt){
        TCut ptCut = Form("(phoEt>=%f)&&(phoEt<%f)", ptBins_i[ipt], ptBins_f[ipt]);
        for(Int_t jcent = 0; jcent < 1; ++jcent){
...
int compareTwo(TTree* t1, TTree* t2, TString var, int nBins, double xMin, double xMax, TCut cut1, TCut cut2, const char* cap)  {
    ...
    int i = 1;
    //static int i = 1;
    TCanvas* c =new TCanvas(Form("c_%s_%s",var.Data(),cap),"", 400,400);
    ...
    //h1->Fit(f1,"Q R");
    //c->GetPad(1)->SetLogy();
    c->SaveAs(Form("figures/ROC_%s_%s.pdf",varName.Data(),cap));
    i++;
    delete c;
    delete h1;
    delete h2;
