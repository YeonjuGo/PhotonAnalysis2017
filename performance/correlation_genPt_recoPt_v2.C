...
//void correlation_genPt_recoPt(TString coll="pp", TString ver="180805_temp_v21", bool isFineBinning=false, bool doWeight=true){
void correlation_genPt_recoPt(TString coll="pbpb", TString ver="180807_temp_v23", bool isFineBinning=false, bool doWeight=true){
   ...
   // TMatrix mymatrix(nbinsx+2,nbinsy+2,myhisto.GetArray(),"F") 
    ...
    for(Int_t j=0;j<nCENTBINS;++j){
        ...
        else h2D_gen_reco[j] = new TH2D(Form("h2D_corr_gen_reco_%s",centSt[j].Data()),";Reco p_{T}^{#gamma} (GeV);Gen p_{T}^{#gamma} (GeV)",nPtBin_unfolding,ptBins_unfolding,nPtBin_unfolding,ptBins_unfolding);
        ...
        for(Int_t i=0;i<4;++i){ /// 0:gen reco-matched 1: gen not reco-matched 2:reco gen-matched 3:reco not gen-matched
            ...
            else h1D_pt[i][j] = new TH1D(Form("h1D_pt_%d_%s",i,centSt[j].Data()),";p_{T}^{#gamma} (GeV);Entries",nPtBin_unfolding, ptBins_unfolding);
        }
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        ...
        //gen (matched)
        if(doWeight)    t1->Draw(Form("%s>>%s","pho_genEt",h1D_pt[0][i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),centCut.GetTitle())); //h1D fineBinning
        else            t1->Draw(Form("%s>>%s","pho_genEt",h1D_pt[0][i]->GetName()), mcIsolation && centCut);
...
//    if(coll=="pp") 
    ...
    /////////////////////////////////////////////////////////////////////
    // Write output file  
    TString outFname = Form("%soutput/correlation_genPt_recoPt_v2_%s.root",dir.Data(),cap.Data());
