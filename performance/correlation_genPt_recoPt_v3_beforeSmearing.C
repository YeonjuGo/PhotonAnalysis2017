...
void correlation_genPt_recoPt_v3_beforeSmearing(TString coll="pp", TString ver="190703_temp_v31_nominal", bool splitMC=false, bool isFineBinning=false, bool doWeight=true){
    cout << " :::::: correlation_genPt_recoPt_v3_beforeSmearing.C :::::: " << endl;
    const TString phoEtVar_noSmear = "phoEtCorrected_beforeSmear";
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
   ...
   // TMatrix mymatrix(nbinsx+2,nbinsy+2,myhisto.GetArray(),"F") 
    ...
    if(splitMC==0){
        ...
        for(Int_t i=0;i<nCENTBINS;++i){
            ...
            //reco (matched)
            if(doWeight)    t1->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[1][i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[1][i]->GetName()), mcIsolation && commonCut);
            h1D_pt[1][i]=(TH1D*)gDirectory->Get(h1D_pt[1][i]->GetName());

            //reco (total, non-matched)
            if(doWeight)    t1->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[2][i]->GetName()), Form("(%s)*((%s))","weight",commonCut.GetTitle()));
            else            t1->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[2][i]->GetName()), commonCut);
            ...
            //h2D fineBinning
            if(doWeight)    t1->Draw(Form("%s:%s>>%s","pho_genEt",phoEtVar_noSmear.Data(),h2D_gen_reco[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1->Draw(Form("%s:%s>>%s","pho_genEt",phoEtVar_noSmear.Data(),h2D_gen_reco[i]->GetName()), mcIsolation && commonCut);
        ...
        for(Int_t i=0;i<nCENTBINS;++i){
            ...
            if(doWeight)    t1_even->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[1][i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1_even->Draw(Form("%s>>%s",phoEtVar_noSmear.Data(),h1D_pt[1][i]->GetName()), mcIsolation && commonCut);
            h1D_pt[1][i]=(TH1D*)gDirectory->Get(h1D_pt[1][i]->GetName());
            ...
            if(doWeight)    t1_odd->Draw(Form("%s:%s>>%s","pho_genEt",phoEtVar_noSmear.Data(),h2D_gen_reco[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut.GetTitle()));
            else            t1_odd->Draw(Form("%s:%s>>%s","pho_genEt",phoEtVar_noSmear.Data(),h2D_gen_reco[i]->GetName()), mcIsolation && commonCut);
            h2D_gen_reco[i]=(TH2D*)gDirectory->Get(h2D_gen_reco[i]->GetName());
            h1D_pt_ratio[i]->Divide(h1D_pt[1][i],h1D_pt[0][i]);
            //h1D_pt_ratio_error[i] = getRatioErrorHist_completelyCorrelated(h1D_pt_ratio[i],h1D_pt[1][i],h1D_pt[0][i]); 
        }
    ...
    TString outFname = Form("%soutput/correlation_genPt_recoPt_%s_beforeSmear.root",dir.Data(),cap.Data());
...
//    c2->Divide(nCENTBINS,2);
    ...
    c2->SaveAs(Form("%sfigures/correlation_genPt_recoPt_1D_%s_beforeSmear.pdf",dir.Data(),cap.Data()));
    ...
    c1->SaveAs(Form("%sfigures/correlation_genPt_recoPt_2D_%s_beforeSmear.pdf",dir.Data(),cap.Data()));
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        ...
        if(coll!="pp"){
            ...
            h2D_gen_reco_percent[i]->Draw("colz text");
            drawText(Form("%d-%d %%",(int)centBins_i[i]/2,(int)centBins_f[i]/2),0.2,1.0-c1->GetTopMargin()-0.06);
            c3_each[i]->SaveAs(Form("%sfigures/correlation_genPt_recoPt_2D_percent_%s_cent%d_beforeSmear.pdf",dir.Data(),cap.Data(),i));
        }
    }
    c3->SaveAs(Form("%sfigures/correlation_genPt_recoPt_2D_percent_%s_beforeSmear.pdf",dir.Data(),cap.Data()));
