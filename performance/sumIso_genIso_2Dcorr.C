...
void sumIso_genIso_2Dcorr(TString ver = "190703_temp_v31"){
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
    SetyjPadStyle();
    ...
    cap += "_hoeCut_sigmaCut";
    cap += "_pt80to200";
    double ymax = 0.05;
    //double ymax = 0.3;
    TH2D* h2D = new TH2D("h2D",";sumIso (GeV);genCalIsoDR4 (GeV)",200,-20,50,180,0.0,5.0);
    ...
    //for MC Bkg
    t1->Draw("pho_genCalIsoDR04:pho_sumIsoCorrected>>h2D",Form("(weight)*(%s && %s && %s && %s && %s)",mcIsolation.GetTitle(), hoeCut.GetTitle(), sigmaCut.GetTitle(), etaCut.GetTitle(), ptCut_.GetTitle()));
   ...
   // proj_pt->Fit(f_pol,"R");
    ...
    c1->SaveAs(Form("figures/2D_corr_sumIso_genIso%s.pdf",cap.Data()));
