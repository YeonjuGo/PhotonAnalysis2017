...
void SCsize_cent_2Dcorr(TString ver = "190703_temp_v31"){
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
    SetyjPadStyle();
    ...
    double ymax = 0.05;
    //double ymax = 0.3;
    TH2D* h2D = new TH2D("h2D",";Centrality;Super cluster #eta width",200,0,100,200,0,ymax);
    t1->Draw("phoSCEtaWidth:hiBin/2.>>h2D",Form("(1.)*(%s && %s && %s && %s && %s)",noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
    //t1->Draw("phoSCPhiWidth:hiBin/2.>>h2D",Form("(1.)*(%s && %s && %s && %s && %s)",noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
    //t1->Draw("phoSCPhiWidth:hiBin/2.>>h2D",Form("(1.)*(%s && %s && %s && %s && %s)",noiseCut.GetTitle(), hoeCut.GetTitle(),sigmaCut.GetTitle(),etaCut.GetTitle(), ptCut_.GetTitle()));
    ...
    c1->SaveAs(Form("figures/2D_corr_SCsize_eta_cent_%s.pdf",cap.Data()));
