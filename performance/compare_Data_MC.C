...
void compare_Data_MC(TString coll="pp", TString ver="190703_temp_v31", bool isBkg=true, double sigCut = 1, double sideband_i = 40, double sideband_f = 50, int Nfunc = 1){
    ...
    for(Int_t i = 0; i < nPtBinIF_; ++i) 
    {
        ...
        for(Int_t k = 0; k < nCENTBINS; ++k){ 
            ...
            for(Int_t j = 0; j < nSumIso; ++j){
                ...
                TCut totCut_data = commonCut && sumIsoCut && trigCut_high && evtSelFilterCut && noiseCut; 
                if(coll=="pp") totCut_data = commonCut && sumIsoCut && trigCut_high && evtSelFilterCut_pp && noiseCut; 
                TCut totCut_mc = commonCut && sumIsoCut && mcBkgIsolation;  
                if(!isBkg) totCut_mc = commonCut && sumIsoCut && mcIsolation;  
                ...
                cout << "start" << endl;
                compareTwo(t1, t2, purityVar,50, 0., 0.030, totCut_data, totCut_mc,cap,cap2,isBkg,Nfunc); 
            }
