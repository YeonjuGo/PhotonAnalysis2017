...
void sumIso_pt_cent_2D(TString ver = "180817_temp_v23", TString coll="pbpb", int sample = 1){
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
    SetyjPadStyle();

    TString cap = ver+"_"+coll+"_etaCut_cent50to100";
    //set file location
    const char* fname;
    if(coll=="pp"){
        if(sample==0){
    ...
    TCut centCut_ = "hiBin>100 && hiBin<=200";
    ...
    t1->Draw("pho_sumIso:phoEtCorrected>>h2D_sumIso_pt",noiseCut && hoeCut && sigmaCut && etaCut && centCut_);
