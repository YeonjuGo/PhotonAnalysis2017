...
void compareFour(TTree* t1=0 ,TTree* t2=0,TString var="pt", int nBins=10, double xMin=0, double xMax=10, TCut cut1="",TCut cut2="", TCut cut3="", TCut cut4="", const char* cap="", const char* cap2="", bool isBkg=true);
void compare_sigmaIEtaIEta_iso_nonIso_4hist_nology(TString coll="pbpb", TString ver="190703_temp_v31", bool isBkg=true){
    ...
    //const double ptBins_i_[] = {25};
    //const double ptBins_f_[] = {30};
    ...
    Int_t nSumIsoCut= 4;
    TCut sumIsoCut[nSumIsoCut];
    sumIsoCut[0] = "pho_sumIsoCorrected<1";
    sumIsoCut[1] = "pho_sumIsoCorrected>=1 && pho_sumIsoCorrected<5";
    sumIsoCut[2] = "pho_sumIsoCorrected>=5 && pho_sumIsoCorrected<10";
    sumIsoCut[3] = "pho_sumIsoCorrected>=10 && pho_sumIsoCorrected<20";
    ...
    for(Int_t i = 0; i < nPtBinIF_; ++i) 
    {
        ...
        for(Int_t k = 0; k < nCENTBINS; ++k){ 
            ...
            for(int j=0;j<nSumIsoCut;++j)
                totCut[j] = commonCut && sumIsoCut[j];
            
            const char* cap = Form("%s_%s_pt%dto%d_cent%dto%d",ver.Data(),coll.Data(),(int)ptBins_i_[i],(int)ptBins_f_[i],(int)centBins_i_[k]/2,(int)centBins_f_[k]/2);
            ...
            cout << "start" << endl;
            if(coll=="pp") compareFour(t1, t1, purityVar_pp,50, 0., 0.030, totCut[0], totCut[1], totCut[2], totCut[3],cap,cap2,isBkg);
            else compareFour(t1, t1, purityVar,50, 0., 0.030, totCut[0], totCut[1], totCut[2], totCut[3],cap,cap2,isBkg); 
            //if(coll=="pp") compareThree(t1, t1, purityVar_pp,50, 0., 0.030, totCut[0], totCut[1], totCut[2],cap,cap2,isBkg);
            //else compareThree(t1, t1, purityVar,50, 0., 0.030, totCut[0], totCut[1], totCut[2],cap,cap2,isBkg); 
        }
    }
} // main function

void compareFour(TTree* t1, TTree* t2, TString var, int nBins, double xMin, double xMax, TCut cut1, TCut cut2, TCut cut3, TCut cut4, const char* cap, const char* cap2, bool isBkg)  {
    SetHistTitleStyle();
    SetyjPadStyle();
    gStyle->SetOptStat(0);
    TH1::SetDefaultSumw2();
    ...
    TH1D* h3 = (TH1D*)h1->Clone(Form("h3_%s_%d",var.Data(),i));
    TH1D* h4 = (TH1D*)h1->Clone(Form("h4_%s_%d",var.Data(),i));
    ...
    t2->Draw(Form("(%s)>>%s",var.Data(),h2->GetName()), Form("(weight)*(%s)",cut2.GetTitle()));
    t2->Draw(Form("(%s)>>%s",var.Data(),h3->GetName()), Form("(weight)*(%s)",cut3.GetTitle()));
    t2->Draw(Form("(%s)>>%s",var.Data(),h4->GetName()), Form("(weight)*(%s)",cut4.GetTitle()));
    ...
    h3->DrawCopy("hist e same");
    h4->DrawCopy("hist e same");
    TLegend* l1 = new TLegend(0.47,0.64,0.92,0.90);
    legStyle(l1);
    if(isBkg){
        l1->AddEntry(h1,"Bkg MC (sumIso<1)","pl");
        l1->AddEntry(h2,"Bkg MC (1<sumIso<5)","pl");
        l1->AddEntry(h3,"Bkg MC (5<sumIso<10)","pl");
        l1->AddEntry(h4,"Bkg MC (10<sumIso<20)","pl");
    } else{
        l1->AddEntry(h1,"Signal MC (sumIso<1)","pl");
        l1->AddEntry(h2,"Signal MC (1<sumIso<5)","l");
        l1->AddEntry(h3,"Signal MC (5<sumIso<10)","l");
        l1->AddEntry(h4,"Signal MC (10<sumIso<20)","l");
    }
    l1->Draw();

    TH1D* h2_ratio = (TH1D*)h1->Clone(Form("h2_ratio_%s_%d",var.Data(),i));
    TH1D* h3_ratio = (TH1D*)h1->Clone(Form("h3_ratio_%s_%d",var.Data(),i));
    TH1D* h4_ratio = (TH1D*)h1->Clone(Form("h4_ratio_%s_%d",var.Data(),i));
    c->cd(2);
    h2_ratio->Divide(h2);
    h3_ratio->Divide(h3);
    h4_ratio->Divide(h4);
    h2_ratio->SetYTitle("Isolated / NonIsolated");
    double ratioRange = getCleverRange(h2_ratio);
    h2_ratio->SetAxisRange(0,3,"Y");

    h2_ratio->SetMarkerColor(2);
    h2_ratio->SetLineColor(2);
    h3_ratio->SetMarkerColor(4);
    h3_ratio->SetLineColor(4);
    h4_ratio->SetMarkerColor(8);
    h4_ratio->SetLineColor(8);

    h2_ratio->DrawCopy("le1");
    h3_ratio->DrawCopy("same le1");
    h4_ratio->DrawCopy("same le1");
   ...
   // fratio->Draw("same");
    ...
    c->SaveAs(Form("%sfigures/compareBKG_%s_%s_%s_nology.pdf",dir.Data(),var.Data(),cap,cap2));
