...
verName_test1=('190703_temp_v31_pur_sumIsoFit_sideband0.010to0.015_sigSub_v2_no2012SigmaVar')
...
##   cp ./purity/output/purity_pbpb_190703_temp_v31_pur_sidebandSigEffat85_SBcorr_centDep_noPtDep_Nfunc0_splitPD_noPreScale.root ./purity/output/purity_pbpb_${verName_test1[$i]}_splitPD_noPreScale.root
    ...
    do
        ...
        then
            ...
            root -l -b -q 'purity/yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit.C++("pp","'${verName_test1[$i]}'",1,0,'${ptBins[$((p))]}','${ptBins[$((p+1))]}',0,0,'$maxiter','$side_i','$side_f')' >& log/yj_quickPhotonPurity_v7_iteration_eachBin_pp_${verName_test1[$i]}_pt${ptBins[$((p))]}to${ptBins[$((p+1))]}.log & sleep 20
            ...
            do
                ...
                then
                    ...
                    root -l -b -q 'purity/yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit.C++("pbpb","'${verName_test1[$i]}'",1,0,'${ptBins[$((p))]}','${ptBins[$((p+1))]}','${centBins[$((j))]}','${centBins[$((j+1))]}','$maxiter','$side_i','$side_f')' >& log/yj_quickPhotonPurity_v7_iteration_eachBin_pbpb_${verName_test1[$i]}_pt${ptBins[$((p))]}to${ptBins[$((p+1))]}_cent${centBins[$((j))]}to${centBins[$((j+1))]}.log & sleep 20
                fi
            done
        fi
    done
