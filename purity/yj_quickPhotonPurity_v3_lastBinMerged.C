...
int yj_quickPhotonPurity_v3_lastBinMerged(const TString coll="pp", const TString ver="test", bool doSplitPD=true, bool doPreScale=1, bool useSBcorr=0, bool noCentDepCorr=0, bool useMCSB=0, bool use30trig=0){
    ...
    if(doSplitPD) outputName+="_splitPD_lastBinMerged";
    if(coll=="pbpb"){
    ...
    //////////////////////////
    //Text File
    ofstream outTxtFile(Form("%stxt/purity_%s.txt",dir.Data(), outputName.Data()), ios::app);
    ...
    TPad* cPurity_each_tempPad[nCENTBINS][nPtBin_unfolding];
    ...
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        ...
        for (Int_t i = nPtBin_unfolding_centDep-2; i <nPtBin_unfolding_centDep-1; ++i) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                //TH1D* htemp = (TH1D*) hSigPdf->Clone(Form("hist_cent%d_pt%d_eta%d",j,k,k));
                //SetHistTextSize(htemp);
                //htemp->DrawCopy("hist");
                ...
                if(coll=="pp") 
                    canName = Form("cPurity_pp_pt%dto%d",(int)ptBins_unfolding[i],(int)ptBins_unfolding[i+1]);
                ...
                cout << "copy pad" << endl;
               // if(coll=="pbpb") cPurity_each_tempPad[j][i]  = (TPad*) cPurity->GetPad((k+j-1)*nPtBin_unfolding+i+1)->Clone(Form("%s_tempPad",canName.Data()));
               // else if(coll=="pp") cPurity_each_tempPad[j][i]  = (TPad*) cPurity->GetPad(i+1)->Clone(Form("%s_tempPad",canName.Data()));
               // cout << "define new canvas" << endl;
               // cPurity_each[j][i] = new TCanvas(Form("%s",canName.Data()),"cPurity_eachBin",300, 300);
               // cout << "draw in canvas" << endl;
               // cPurity_each_tempPad[j][i]->Draw();
               // cPurity_each_tempPad[j][i]->SetPad(0,0,1,1);
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v3_lastBinMerged(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v3_lastBinMerged(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v3_lastBinMerged(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v3_lastBinMerged(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
