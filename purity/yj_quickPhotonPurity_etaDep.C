...
#include "PhotonPurity_SBcorrection_etaDep.h"
//#include "../phoRaaCuts/phoRaaCuts_purity_forPaper.h"
#include "../phoRaaCuts/phoRaaCuts_purity_etaDep.h"
...
int yj_quickPhotonPurity_etaDep(const TString coll="pp", const TString ver="etaDep_180807_temp_v22", bool doSplitPD=true, bool doPreScale=0, bool useSBcorr=0, bool noCentDepCorr=0, bool useMCSB=0){
    ...
    TString inputMC=""; 
    TString inputBkgMC ="";
    TString outputName=Form("purity_%s_%s_cent%dto%d",coll.Data(),ver.Data(),centBins_i[0]/2,centBins_f[0]/2);
   ...
   // std::string trigger = configCuts.proc[CUTS::kHISTOGRAM].obj[CUTS::kPHOTON].s[CUTS::PHO::k_trigger_gammaJet].c_str();
    ...
    for (Int_t k = 0; k < nEtaBin; k++) {
        if(coll=="pp") h1D_pur[j][k] = new TH1D(Form("h1D_purity_pp_eta%d",k),";p_{T}^#gamma (GeV);",nPtBin,ptBins_draw);
        else h1D_pur[j][k] = new TH1D(Form("h1D_purity_cent%d_eta%d",j,k),";p_{T}^#gamma (GeV);",nPtBin,ptBins_draw);
    }
    }

    cout << "nPtBin = " << nPtBin << " , nCentBins = " << nCENTBINS << ", nEtaBin = " << nEtaBin << endl;
    TCanvas* cPurity = new TCanvas("c1", "c1", 400*nPtBin, 400*nEtaBin);
    makeMultiPanelCanvas(cPurity, nPtBin, nEtaBin, 0.0, 0.0 , 0.2, 0.15, 0.005);
    ...
    for (Int_t i = 0; i < nPtBin; ++i) {
        for (Int_t j = 0; j < nCENTBINS; ++j) {
            for (Int_t k = 0; k < nEtaBin; ++k) {
                ...
                cout << " Range ::: " << ptCut_ << ", " << centCut << ", " << etaCut_ << endl; 
                cout << " Trigger ::: " << trigCut_.GetTitle() << endl; 

                TCut dataCandidateCut = trigCut_ && sampleIsolation_woTrig && etaCut_ && ptCut_ && centCut;
                if(coll=="pp") dataCandidateCut = trigCut_ && sampleIsolation_woTrig_pp && etaCut_ && ptCut_;

                TCut sidebandCut = ""; 
                if(useMCSB==0 && coll=="pbpb") sidebandCut = trigCut_ && sidebandIsolation_woTrig && etaCut_ && ptCut_ && centCut;
                else if(useMCSB==0 && coll=="pp") sidebandCut = trigCut_ && sidebandIsolation_woTrig_pp && etaCut_ && ptCut_;
                else if(useMCSB==1 && coll=="pbpb") sidebandCut = mcBkgIsolation && mcCut && hoeCut && isoCut && etaCut_ && ptCut_ && centCut;
                else if(useMCSB==1 && coll=="pp") sidebandCut = mcBkgIsolation && mcCut_pp && hoeCut_pp && isoCut_pp && etaCut_ && ptCut_;
                else cout << "something's wrong" << endl; 

                TCut mcSignalCut = mcSignalCut_woKine && etaCut_ && ptCut_ && centCut;
               ...
               // TH1D* hData1  = fitr->hdata;
                ...
                if (i == 0) {
                    ...
                    if(coll=="pbpb") drawText(Form("%.2f < |#eta^{#gamma}| < %.2f", etaBins[k], etaBins[k+1]), xpos, 0.40, 1, 27);
               ...
               // }
                ...
                //h1D_pur[j][k]->SetBinError(i+1,(Float_t)fitr.purity);
            }
        }
    }
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_etaDep(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_etaDep(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_etaDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_etaDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
