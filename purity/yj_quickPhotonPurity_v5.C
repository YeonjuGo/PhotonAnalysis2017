...
// v5) fitting variable with sumIso and sigmaIEtaIEta as sideband  
...
#include "PhotonPurity_sumIsoFit.h"
...
int yj_quickPhotonPurity_v5(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, bool useSBcorr=0, bool centDepCorr=0, int ptDepCorrThr=0, int Nfunc=0, bool useMCSB=0, bool use30trig=0){
    ...
    TH1D* htemp = new TH1D(Form("htemp%d",1),";#sigma_{#eta#eta};Entries",100,-20,60);
    ...
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        ...
        for (Int_t i = 0; i < nPtBin_unfolding_centDep-1; ++i) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                TCut dataCandidateCut = trigCut_ && preScaleCut && noiseCut && hoeCut && sigmaCut && electronCut && etaCut && ptCut_ && centCut;
                if(coll=="pp") dataCandidateCut = trigCut_ && preScaleCut && noiseCut && hoeCut_pp && sigmaCut_pp && electronCut && etaCut && ptCut_;

                TCut sidebandCut = ""; 
                TCut tempSBcut = "";
                if(useMCSB==0 && coll=="pbpb"){ 
                   // int tempSumIso_pp_i = 0.2*ptBins_unfolding[i]-2;
                   // int tempSumIso_pp_f = 0.2*ptBins_unfolding[i]-2+SB_interval_pp;
                   // tempSBcut = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f", 
                   //             0.2*ptBins_unfolding[i]-2, 0.2*ptBins_unfolding[i]-2+SB_interval_pp);
                    tempSBcut = Form("phoSigmaIEtaIEta_2012> %f && phoSigmaIEtaIEta_2012< %f",0.0100,0.0115); 
...
//                                0.2*ptBins_unfolding[i]-2, 0.2*ptBins_unfolding[i]-2+SB_interval_pp);
                ...
                TCut mcSignalCut = mcIsolation && mcCut && hoeCut && sigmaCut && etaCut && ptCut_ && centCut;
                if(coll=="pp") mcSignalCut = mcIsolation && mcCut_pp && hoeCut_pp && sigmaCut_pp && etaCut && ptCut_;

                ////////////////////////////////////////////////////////////////
                // SB correction file part 
                TString fname_SBcorr = "";
                //TString cap_Nfunc = "";
                //if(coll=="pp") cap_Nfunc += Form("_fitRange0p008to0p015%s","");
                ...
                //    int tempSumIso_pp_i = 0.2*ptBins_unfolding[i]-2;
                //    int tempSumIso_pp_f = 0.2*ptBins_unfolding[i]-2+SB_interval_pp;
                //    TString cap_purHere = Form("sigSumIso1_sidebandSumIso%dto%d",tempSumIso_pp_i,tempSumIso_pp_f);
                //    if(centDepCorr==0){
                ...
                //        if(ptDepCorrThr==0) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to200_cent%dto%d_bkgPhotons_%s%s.root",(int)centBins_i[j]/2,(int)centBins_f[j]/2,cap_purHere.Data(),cap_Nfunc.Data()); 
                //        else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]<ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to%d_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,(int)centBins_i[j]/2,(int)centBins_f[j]/2,cap_purHere.Data(),cap_Nfunc.Data());
                //        else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]>=ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt%dto200_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,(int)centBins_i[j]/2,(int)centBins_f[j]/2,cap_purHere.Data(),cap_Nfunc.Data());
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v5(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v5(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v5(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v5(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
