...
int yj_quickPhotonPurity_v7_iteration_separateCent_inputIter(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, int centHere = 1, double sideband_i = 10, double sideband_f = 20, int iter_i=0, int iter_f=3, bool useSBcorr=0, bool centDepCorr=0, int ptDepCorrThr=0, int Nfunc=0, bool useMCSB=0, bool use30trig=0){
    cout << "Purity Calculation using yj_quickPhotonPurity_v7_iteration_separateCent_inputIter.C" << endl;
    ...
    outputName+=Form("_cent%d",centHere);
    ...
    Int_t nCENTBINS = centHere+1;
    if(coll=="pp") nCENTBINS=1;
    if(coll=="pp") centHere=0;
    ...
    TString cap_inf = "";
    if(doSplitPD) cap_inf+="_splitPD";
    if(!doPreScale) cap_inf+="_noPreScale";
    TFile* inputPurF;
    TH1D* h1D_inputPur; 
    if(iter_i!=0){
        inputPurF = new TFile(Form("%soutput/purity_%s_%s_cent%d%s.root",dir.Data(),coll.Data(),ver.Data(),centHere,cap_inf.Data()), "READ");
        if(coll=="pp")
            h1D_inputPur = (TH1D*) inputPurF->Get(Form("h1D_purity_pp_iter%d",iter_i-1));
        else
            h1D_inputPur = (TH1D*) inputPurF->Get(Form("h1D_purity_cent%d_iter%d",centHere,iter_i-1));
    }

    //purity_pp_190703_temp_v31_pur_maxiter3_sideband5to10_cent1_splitPD_noPreScale.root
    TH1D* h1D_pur[nCENTBINS][5];
    TH1D* htemp = new TH1D(Form("htemp%d",1),";#sigma_{#eta#eta};Entries",100,0.001,0.024);
    TCanvas* cPurity[5];
    TCanvas* cPurity_each[nCENTBINS][nPtBin_unfolding][5];
    for (Int_t j = centHere; j < nCENTBINS; j++) {
        ...
        for (Int_t it = 0;it < maxPurIter[j]; ++it) {
            if(coll=="pp") h1D_pur[j][it] = new TH1D(Form("h1D_purity_pp_iter%d",it),";p_{T}^#gamma (GeV);",nPtBin_unfolding_centDep,ptBins_unfolding_centDep);
            else h1D_pur[j][it] = new TH1D(Form("h1D_purity_cent%d_iter%d",j,it),";p_{T}^#gamma (GeV);",nPtBin_unfolding_centDep,ptBins_unfolding_centDep);
            cPurity[it] = new TCanvas(Form("c1_%d",it), "c1", 300*(nPtBin_unfolding-1), 300);
            makeMultiPanelCanvas(cPurity[it],(nPtBin_unfolding-1) , 1, 0.0, 0.0 , 0.2, 0.15, 0.005);
        }
    }

    cout << "nPtBin_unfolding = " << nPtBin_unfolding << " , centBin = " << centHere << endl;
    ...
    for (Int_t j = centHere; j < nCENTBINS; ++j) {
        ...
        for (Int_t i = 0; i < nPtBin_unfolding_centDep-1; ++i) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                TCut sidebandCut = ""; 
                TCut nonIsoSBCut_temp = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f",(double)sideband_i,(double)sideband_f);
                if(useMCSB==0 && coll=="pbpb") sidebandCut = trigCut_ && preScaleCut && noiseCut && hoeCut && nonIsoSBCut_temp && electronCut && etaCut && ptCut_ && centCut;
                else if(useMCSB==0 && coll=="pp") sidebandCut = trigCut_ && preScaleCut && noiseCut && hoeCut_pp && nonIsoSBCut_temp && electronCut && etaCut && ptCut_;
                ...
                cout << "Sideband Range :: " << nonIsoSBCut_temp.GetTitle() << endl;
                TCut mcSignalCut = mcSignalCut_woKine && etaCut && ptCut_ && centCut;
                if(coll=="pp") mcSignalCut = mcSignalCut_woKine_pp && etaCut && ptCut_;
                TCut mcSignalSBCut = mcIsolation && mcCut && hoeCut && nonIsoSBCut_temp && etaCut && ptCut_ && centCut;
                if(coll=="pp") mcSignalSBCut = mcIsolation && hoeCut_pp && nonIsoSBCut_temp && etaCut && ptCut_ && centCut;
                ...
                double purityResult[iter_f];
                for (Int_t it = iter_i;it < iter_f; ++it) {
                    
                    PhotonPurity fitr;

                    double tempInputPurity = 0.0;
                    if(it!=0)  tempInputPurity = purityResult[it-1];
                    if(iter_i!=0 && it==iter_i) tempInputPurity = h1D_inputPur->GetBinContent(i+1); 
                    cout << "inputPurity = " << tempInputPurity << endl;

                    TString dir = "/home/goyeonju/CMS/2017/PhotonAnalysis2017/purity/";
                    TString tempOutputName = outputName;
                    tempOutputName+=Form("_cPurity_cent%dto%d_pt%dto%d_iter%d",(int)centBins_i[j]/2,(int)centBins_f[j]/2,(int)ptBins_unfolding[i],(int)ptBins_unfolding[i+1],it);

                    if(doSplitPD && ptBins_unfolding[i]>=40){
                        if(useMCSB==0 && useSBcorr==0){
                            fitr = getPurity(coll, configCuts, tPho_high, tmcPho,
                                    dataCandidateCut, sidebandCut,
                                    mcSignalCut, preScaleLabel,tempInputPurity,it,mcSignalSBCut,tempOutputName);
                    ...
                    TString canName = Form("cPurity_cent%dto%d_pt%dto%d_iter%d",(int)centBins_i[j]/2,(int)centBins_f[j]/2,(int)ptBins_unfolding[i],(int)ptBins_unfolding[i+1],it);
                    if(i==nPtBin_unfolding_centDep-2) 
                        canName = Form("cPurity_cent%dto%d_pt%dto%d_iter%d",(int)centBins_i[j]/2,(int)centBins_f[j]/2,(int)ptBins_unfolding[i],(int)ptBins_unfolding[i+2],it);

                    cPurity_each[j][i][it] = new TCanvas(Form("%s",canName.Data()),"cPurity_eachBin",300, 300);
                    htemp->Draw();
                    ...
                    cPurity_each[j][i][it]->SaveAs(Form("%sfigures/%s_%s.pdf",dir.Data(),outputName.Data(),canName.Data()));

                    h1D_pur[j][it]->SetBinContent(i+1,(Float_t)fitr.purity);
                    h1D_pur[j][it]->SetBinError(i+1,(Float_t)fitr.purityErr);
    ...
    for (Int_t j = centHere; j < nCENTBINS; ++j) {
        for (Int_t it = 0; it < maxPurIter[j]; ++it) {
            h1D_pur[j][it]->Write();
            cPurity[it]->Write();
            cPurity[it]->SaveAs(Form("%sfigures/%s_iter%d.pdf",dir.Data(), outputName.Data(),it));
        }
    }
    outFile->Close();
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v7_iteration_separateCent_inputIter(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v7_iteration_separateCent_inputIter(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v7_iteration_separateCent_inputIter(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v7_iteration_separateCent_inputIter(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
