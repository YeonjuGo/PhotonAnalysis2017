...
int yj_quickPhotonPurity_v4_temp(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, bool useSBcorr=0, bool centDepCorr=0, int ptDepCorrThr=0, int Nfunc=0, bool useMCSB=0, bool use30trig=0){
    ...
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        ...
        for (Int_t i = 0; i < nPtBin_unfolding_centDep-1; ++i) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                TCut sidebandCut = ""; 
                TCut tempSBcut = "";
                double SB_interval_pp_Here = 5.0;
                if(useMCSB==0 && coll=="pbpb"){ 
                    double tempSumIso_i = 0.1*ptBins_unfolding[i];
                    double tempSumIso_f = 0.1*ptBins_unfolding[i]+SB_interval_pp_Here;
                ...
                if(useSBcorr==1 && coll=="pp"){
                    ...
                    //int tempSumIso_pp_f = 0.2*ptBins_unfolding[i]-2+SB_interval_pp_Here;
                    TString cap_purHere = Form("sigSumIso1_sidebandSumIso%.1fto%.1f",tempSumIso_i,tempSumIso_f);
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v4_temp(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v4_temp(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v4_temp(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v4_temp(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
