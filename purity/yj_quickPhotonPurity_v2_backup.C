...
int yj_quickPhotonPurity_v2(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=1, bool useSBcorr=0, bool noCentDepCorr=0, bool useMCSB=0){
   ...
   // std::string trigger = configCuts.proc[CUTS::kHISTOGRAM].obj[CUTS::kPHOTON].s[CUTS::PHO::k_trigger_gammaJet].c_str();
    ...
    makeMultiPanelCanvas(cPurity, nPtBin_unfolding, nCENTBINS, 0.0, 0.0 , 0.2, 0.15, 0.005);
    for (Int_t i = 0; i < nPtBin_unfolding; ++i) {
        for (Int_t j = 0; j < nCENTBINS; ++j) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                cPurity->cd((k+j)*nPtBin_unfolding+i+1);
