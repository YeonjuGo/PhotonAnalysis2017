...
int yj_quickPhotonPurity_v3_etaDep(const TString coll="pbpb", const TString ver="190625_temp_v29_purityEtaDep", bool doSplitPD=true, bool doPreScale=1, bool useSBcorr=0, bool noCentDepCorr=0, bool useMCSB=0, bool use30trig=0){
    ...
    for (Int_t j = 0; j < nCENTBINS; j++) {
        for (Int_t k = 0; k < nEtaBin; k++) {
            if(coll=="pp") h1D_pur[j][k] = new TH1D(Form("h1D_purity_pp_eta%d",k),";p_{T}^#gamma (GeV);",nPtBin,ptBins);
            else h1D_pur[j][k] = new TH1D(Form("h1D_purity_cent%d_eta%d",j,k),";p_{T}^#gamma (GeV);",nPtBin,ptBins);
        }
    }


    cout << "nPtBin = " << nPtBin << " , nCentBins = " << nCENTBINS << ", nEtaBins = " << nEtaBin << endl;
    TCanvas* cPurity[nCENTBINS];
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        cPurity[j] = new TCanvas(Form("c1%d",j), "c1", 300*nPtBin, 300*nEtaBin);
        makeMultiPanelCanvas(cPurity[j],nPtBin , nEtaBin, 0.0, 0.0 , 0.2, 0.15, 0.005);
    }
    TCanvas* cPurity_each[nCENTBINS][nPtBin][nEtaBin];


    for (Int_t j = 0; j < nCENTBINS; ++j) {
        for (Int_t i = 0; i < nPtBin; ++i) {
            for (Int_t k = 0; k < nEtaBin; ++k) {
                cout << "START_CENTBIN_" << centBins_i[j] << "_to_" << centBins_f[j] << " AND ETABIN_ " << etaBins_i[k] << "_to_" << etaBins_f[k] << endl;

                TString ptCut_ = Form("(%s>= %f) && (%s< %f)",
                        phoEtVar.Data(), ptBins[i], phoEtVar.Data(), ptBins[i+1]);

                TString centCut_ = Form("(hiBin>=%i) && (hiBin<%i)",
                        centBins_i[j], centBins_f[j]);

                TString etaCut_ = Form("abs(phoEta)>=%f && abs(phoEta)<%f",
                        etaBins_i[k], etaBins_f[k]);
                ...
                if(doSplitPD){
                    ...
                    } else if(ptBins[i]>=30 && ptBins[i]<40){ // 30-40 GeV, lower pt 
                    ...
                    } else if(ptBins[i]>=20 && ptBins[i]<30){ // 20-30 GeV, lower pt 
                        if(coll=="pp"){ 
                            trigCut_ = trigCut_pp_low; 
                            preScaleLabel = "HLT_HISinglePhoton20_Eta1p5_v1_Prescl * L1_SingleEG5_BptxAND_Prescl";
                            preScaleCut = "1"; 
                ...
                cout << " Range ::: " << ptCut_ << ", " << centCut_ << ", " << etaCut_ << endl; 
                cout << " Trigger ::: " << trigCut_.GetTitle() << endl; 

                TCut dataCandidateCut = trigCut_ && preScaleCut && sampleIsolation_woTrig && etaCut_ && ptCut_ && centCut_;
                if(coll=="pp") dataCandidateCut = trigCut_ && preScaleCut && sampleIsolation_woTrig_pp && etaCut_ && ptCut_;

                TCut sidebandCut = ""; 
                if(useMCSB==0 && coll=="pbpb") sidebandCut = trigCut_ && preScaleCut && sidebandIsolation_woTrig && etaCut && ptCut_ && centCut_;
                else if(useMCSB==0 && coll=="pp") sidebandCut = trigCut_ && preScaleCut && sidebandIsolation_woTrig_pp && etaCut && ptCut_;
                else if(useMCSB==1 && coll=="pbpb") sidebandCut = mcBkgIsolation && mcCut && hoeCut && isoCut && etaCut && ptCut_ && centCut_;
                ...
                TCut mcSignalCut = mcSignalCut_woKine && etaCut_ && ptCut_ && centCut_;
                if(coll=="pp") mcSignalCut = mcSignalCut_woKine_pp && etaCut_ && ptCut_;
                ...
                if (i == 0) {
                    ...
                    drawText(Form("%.2f < |#eta^{#gamma}| < %.2f", etaBins_i[k], etaBins_f[k]), xpos, 0.40, 1, 27);
                    if(coll=="pbpb") drawText(Form("%.0f - %.0f%c", centBins_i[j]/2., centBins_f[j]/2., '%'), xpos, 0.30, 1, 27);
                }
                ...
                outTxtFile << (int)centBins_i[j]/2 << "\t" << (int)centBins_f[j]/2 << "\t" << (int)ptBins[i] << "\t" << (int)ptBins[i+1] << "\t" << (Float_t)fitr.purity << "\t" << (Float_t)fitr.purityErr  << endl;

                TString canName = Form("cPurity_cent%dto%d_eta%d_pt%dto%d",(int)centBins_i[j]/2,(int)centBins_f[j]/2,k,(int)ptBins[i],(int)ptBins[i+1]);

                cPurity_each[j][i][k] = new TCanvas(Form("%s",canName.Data()),"cPurity_eachBin",300, 300);
                htemp->Draw();
                hSigPdf->DrawCopy("same hist");
                hBckPdf->DrawCopy("same hist");
                hData1->DrawCopy("same p");
                xpos = 0.45;
                drawText(Form("%.0f GeV/c < p_{T}^{#gamma} < %.0f GeV/c", ptBins[i], ptBins[i+1]), 0.15, 0.90);  
                ...
                drawText(Form("%.2f < |#eta^{#gamma}| < %.2f", etaBins_i[k], etaBins_f[k]), xpos, 0.50);
                if(coll=="pbpb") drawText(Form("%.0f - %.0f%c", centBins_i[j]/2., centBins_f[j]/2., '%'), xpos, 0.40);

                cPurity_each[j][i][k]->SaveAs(Form("%sfigures/%s_%s.pdf",dir.Data(),outputName.Data(),canName.Data()));

                h1D_pur[j][k]->SetBinContent(i+1,(Float_t)fitr.purity);
                h1D_pur[j][k]->SetBinError(i+1,(Float_t)fitr.purityErr);
            }
        }
    }
    ...
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        cPurity[j]->Write();
        cPurity[j]->SaveAs(Form("%sfigures/%s.pdf",dir.Data(), outputName.Data()));
        for (Int_t k = 0; k < nEtaBin; ++k) 
            h1D_pur[j][k]->Write();
    }
    outFile->Close();
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v3_etaDep(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v3_etaDep(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v3_etaDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v3_etaDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
