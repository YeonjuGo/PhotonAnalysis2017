...
PhotonPurity getPurity(const TString coll, CutConfiguration config, TTree *dataTree, TTree *mcTree,
		       ...
		       TCut mcSignalCut, TCut dataWeightLabel, double inputPurity = 0.0, int niter=0, TCut mcSignalSBCut = "",
               TString outputName ="", TString fname_SBcorr="", TTree *bkgmcTree=0)
{
  ...
  std::cout << "sigCount: " << Nsig << std::endl;

  double purityBinVal = 0.010;
  if(inputPurity!=0 && niter!=0){
      ...
      double Nsig_sb = hSig_sb->GetEntries();
      double relRatio = (hSig_sb->Integral(1, hSig_sb->FindBin(purityBinVal)))/(hSig->Integral(1, hSig->FindBin(purityBinVal)));
      std::cout << "sigInSBCount: " << Nsig_sb << std::endl;
      std::cout << "sig MC relative Ratio: " << relRatio << ", Nsig_sb/Nsig = " << Nsig_sb/Nsig << std::endl;

      Double_t DATAsig = hCand->Integral(1, hCand->FindBin(purityBinVal));
      hBkg_temp->Scale((DATAsig*(1.0-inputPurity)/hBkg_temp->Integral(1, hBkg_temp->FindBin(purityBinVal))));
      hSig_sb->Scale(DATAsig*inputPurity*relRatio/hSig_sb->Integral(1,hSig_sb->FindBin(purityBinVal)));
      ...
      std::cout << "Input purity of " << niter << " th iteration : " << inputPurity << std::endl;
