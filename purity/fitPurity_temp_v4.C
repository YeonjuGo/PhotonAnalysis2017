...
void fitPurity_temp_v4(const TString coll="pbpb", const TString ver="190625_temp_v29_sys_purDown_test", bool doSplitPD=true, bool drawGraph = true, const int function_number = 0){
    ...
    TString outfName=Form("purity_v4_withFunc_%s_%s_splitPD_noPreScale",coll.Data(),ver.Data());
    TString infName=Form("purity_v4_%s_%s_splitPD_noPreScale",coll.Data(),ver.Data());
    ...
    for (Int_t j = 0; j < nCENTBINS; j++) {
        ...
        if(function_number==0){
            ...
            if(coll=="pbpb") f_temp[j]->SetParLimits(0,0.65,0.85);
            //if(coll=="pbpb") f_temp[j]->SetParLimits(0,0.65,0.90);
            //if(coll=="pbpb") f_temp[j]->SetParLimits(0,0.65,0.95);
            if(coll=="pbpb") f_temp[j]->SetParLimits(1,-400,-50);
            if(coll=="pbpb") f_temp[j]->SetParLimits(2,100,300);
