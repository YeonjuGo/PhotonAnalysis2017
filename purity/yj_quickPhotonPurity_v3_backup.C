...
int yj_quickPhotonPurity_v3(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=1, bool useSBcorr=0, bool noCentDepCorr=0, bool useMCSB=0, bool use30trig=0){
    ...
    TCanvas* cPurity = new TCanvas("c1", "c1", 300*(nPtBin_unfolding-1), 300*(nCENTBINS-1));
    if(coll=="pbpb") makeMultiPanelCanvas(cPurity, nPtBin_unfolding, nCENTBINS-1, 0.0, 0.0 , 0.2, 0.15, 0.005);
    else if(coll=="pp") makeMultiPanelCanvas(cPurity, nPtBin_unfolding, 1, 0.0, 0.0 , 0.2, 0.15, 0.005);
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v3(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v3(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v3(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v3(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
