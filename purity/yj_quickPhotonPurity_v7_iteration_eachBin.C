...
// yj_quickPhotonPurity_v7_iteration_eachBin) Calculate purity only for one bin with several iteration! 
...
int yj_quickPhotonPurity_v7_iteration_eachBin(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, double pt_i=20, double pt_f=25, int cent_i=0, int cent_f=10, int maxIter=10, double sideband_i=1, double sideband_f=3){
    cout << "Purity Calculation using yj_quickPhotonPurity_v7_iteration_eachBin.C" << endl;
    ...
    for (Int_t it = 0; it < maxIter; ++it) {
        ...
        cout << " Input purity    ::: " << tempInputPurity << endl; 
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v7_iteration_eachBin(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v7_iteration_eachBin(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v7_iteration_eachBin(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v7_iteration_eachBin(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
