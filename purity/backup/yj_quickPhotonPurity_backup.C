...
int yj_quickPhotonPurity(const TString configFile, const TString inputData, const TString inputMC, const TString outputName, bool useMCSB) {
    gStyle->SetOptStat(0);
    ...
    TString inputBkgMC = pbpbMCEmEnrfname; 
    if(collisionType==2) inputBkgMC = ppMCEmEnrfname; 
    ...
    for (Int_t i = 0; i < nPtBin; ++i) {
        for (Int_t j = 0; j < nCENTBINS; ++j) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                if(collisionType==2) centCut == "(1==1)";
...
int main(int argc, char* argv[]) {
    if (argc == 5)
        return yj_quickPhotonPurity(argv[1], argv[2], argv[3], argv[4], 0);
    if (argc == 6)
        return yj_quickPhotonPurity(argv[1], argv[2], argv[3], argv[4], atoi(argv[5]));
    return 1;
}
