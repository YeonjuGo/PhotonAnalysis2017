...
// yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit) Calculate purity only for one bin with several iteration! 
...
#include "PhotonPurity_sigSub_iteration_v2_sumIsoFit.h"
...
int yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, double pt_i=20, double pt_f=25, int cent_i=0, int cent_f=10, int maxIter=2, double sideband_i=0.010, double sideband_f=0.015){
    cout << "Purity Calculation using yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit.C" << endl;
    ...
    TH1D* htemp = new TH1D(Form("htemp%d",1),";sumIso;Entries",150,-50,100);
    ...
    // Cuts 
    //
    TCut dataCandidateCut = trigCut_ && preScaleCut && noiseCut && hoeCut && sigmaCut && electronCut && etaCut && ptCut_ && centCut_;
    if(coll=="pp") dataCandidateCut = trigCut_ && preScaleCut && noiseCut && hoeCut_pp && sigmaCut_pp && electronCut && etaCut && ptCut_;

    TCut mcSignalCut = mcIsolation && mcCut && hoeCut && sigmaCut && etaCut && ptCut_ && centCut_;
    if(coll=="pp") mcSignalCut = mcIsolation && mcCut_pp && hoeCut_pp && sigmaCut_pp && etaCut && ptCut_;

    TCut nonIsoSBCut_here = Form("phoSigmaIEtaIEta> %f && phoSigmaIEtaIEta< %f",(double)sideband_i,(double)sideband_f);
    //TCut nonIsoSBCut_here = Form("phoSigmaIEtaIEta_2012> %f && phoSigmaIEtaIEta_2012< %f",(double)sideband_i,(double)sideband_f);
    cout << " Sideband Range    ::: " << nonIsoSBCut_here.GetTitle() << endl; 
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v7_iteration_eachBin_v2_sumIsoFit(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
