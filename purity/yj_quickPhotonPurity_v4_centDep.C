...
// v4) different SB region and SB correction files  
...
int yj_quickPhotonPurity_v4_centDep(const TString coll="pp", const TString ver="phoRaaCuts_purity_forPaper", bool doSplitPD=true, bool doPreScale=0, bool useSBcorr=0, int centDepCorrThr=30, int ptDepCorrThr=0, int Nfunc=0, double SB_interval=5, bool useMCSB=0, bool use30trig=0){
    ...
    cout << "::: centDepCorrThr = " << centDepCorrThr << endl; 
    ...
    for (Int_t j = 0; j < nCENTBINS; ++j) {
        ...
        for (Int_t i = 0; i < nPtBin_unfolding_centDep-1; ++i) {
            for (Int_t k = 0; k < 1; ++k) {
                ...
                TCut sidebandCut = ""; 
                TCut tempSBcut = "";
                if(useMCSB==0 && coll=="pbpb"){ 
                    double tempSumIso_i = 0.2*ptBins_unfolding[i]-2;
                    double tempSumIso_f = 0.2*ptBins_unfolding[i]-2+SB_interval;
                    //double tempSumIso_i = 0.1*ptBins_unfolding[i];
                    //double tempSumIso_f = 0.1*ptBins_unfolding[i]+SB_interval;
                    tempSBcut = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f", 
                                tempSumIso_i, tempSumIso_f);

                    cout << "Sideband Ragne :: " << tempSBcut.GetTitle() << endl;
                    //if(j==1) tempSBcut = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f",SB_pbpb_i[j],SB_pbpb_f[j]);
                    //else if(j==2) tempSBcut = Form("pho_sumIsoCorrected >%f && pho_sumIsoCorrected < %f",SB_pbpb_i[j],SB_pbpb_f[j]);
                    //else if(j==3) tempSBcut = Form("pho_sumIsoCorrected >%f && pho_sumIsoCorrected <%f",SB_pbpb_i[j],SB_pbpb_f[j]);
                    //else if(j==4) tempSBcut = Form("pho_sumIsoCorrected >%f && pho_sumIsoCorrected <%f",SB_pbpb_i[j],SB_pbpb_f[j]);
                    sidebandCut = trigCut_ && preScaleCut && noiseCut && hoeCut && tempSBcut && electronCut && etaCut && ptCut_ && centCut;
                } else if(useMCSB==0 && coll=="pp"){ 
                    double tempSumIso_i = 0.2*ptBins_unfolding[i]-2;
                    double tempSumIso_f = 0.2*ptBins_unfolding[i]-2+SB_interval;
                    //double tempSumIso_i = 0.1*ptBins_unfolding[i];
                    //double tempSumIso_f = 0.1*ptBins_unfolding[i]+SB_interval;
                    tempSBcut = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f", 
                                tempSumIso_i, tempSumIso_f);
                    //tempSBcut = Form("pho_sumIsoCorrected > %f && pho_sumIsoCorrected < %f",10.0,20.0);
                    cout << "Sideband Ragne :: " << tempSBcut.GetTitle() << endl;
                    sidebandCut = trigCut_ && preScaleCut && noiseCut && hoeCut && tempSBcut && electronCut && etaCut && ptCut_;
                    //sidebandCut = trigCut_ && preScaleCut && sidebandIsolation_woTrig_pp && etaCut && ptCut_;
                ...
                TString cap_Nfunc = "";
                cap_Nfunc += Form("_fitRange0p008to0p015%s","");
                cap_Nfunc += Form("_function%d",Nfunc);
                ...
                if(useSBcorr==1 && coll=="pbpb"){
                    ...
                    //double tempSumIso_i = 0.1*ptBins_unfolding[i];
                    //double tempSumIso_f = 0.1*ptBins_unfolding[i]+SB_interval;
                    TString cap_purHere = Form("sigSumIso1_sidebandSumIso%dto%d",(int)tempSumIso_i,(int)tempSumIso_f);
                    //TString cap_purHere = Form("sigSumIso1_sidebandSumIso%.1fto%.1f",tempSumIso_i,tempSumIso_f);

                    if(centDepCorrThr==0){
                        if(ptDepCorrThr==0) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to200_cent0to100_bkgPhotons_%s%s.root",cap_purHere.Data(),cap_Nfunc.Data()); 
                        else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]<ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to%d_cent0to100_bkgPhotons_%s%s.root",ptDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());
                        else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]>=ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt%dto200_cent0to100_bkgPhotons_%s%s.root",ptDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());
                    } else{//cent dep
                        //cap_purHere = Form("sigSumIso1_sidebandSumIso%dto%d",(int)SB_pbpb_i[j],(int)SB_pbpb_f[j]);

                        if(centBins_i[j]/2<centDepCorrThr){

                            if(ptDepCorrThr==0) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to200_cent%dto%d_bkgPhotons_%s%s.root",0,centDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data()); 
                            else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]<ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to%d_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,0,centDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());
                            else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]>=ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt%dto200_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,0,centDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());

                        } else{

                            if(ptDepCorrThr==0) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to200_cent%dto%d_bkgPhotons_%s%s.root",centDepCorrThr,100,cap_purHere.Data(),cap_Nfunc.Data()); 
                            else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]<ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt20to%d_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,centDepCorrThr,100,cap_purHere.Data(),cap_Nfunc.Data());
                            else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]>=ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pbpb_pt%dto200_cent%dto%d_bkgPhotons_%s%s.root",ptDepCorrThr,centDepCorrThr,100,cap_purHere.Data(),cap_Nfunc.Data());
                ...
                if(useSBcorr==1 && coll=="pp"){
                    //double tempSumIso_i = 0.1*ptBins_unfolding[i];
                    //double tempSumIso_f = 0.1*ptBins_unfolding[i]+SB_interval;
                    //int tempSumIso_pp_i = 10; 
                    //int tempSumIso_pp_f = 20; 
                    int tempSumIso_i = 0.2*ptBins_unfolding[i]-2;
                    int tempSumIso_f = 0.2*ptBins_unfolding[i]-2+SB_interval;
                    TString cap_purHere = Form("sigSumIso1_sidebandSumIso%dto%d",tempSumIso_i,tempSumIso_f);
                    //TString cap_purHere = Form("sigSumIso1_sidebandSumIso%.1fto%.1f",tempSumIso_i,tempSumIso_f);
                    if(ptDepCorrThr==0) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pp_pt20to200_bkgPhotons_%s%s.root",cap_purHere.Data(),cap_Nfunc.Data()); 
                    else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]<ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pp_pt20to%d_bkgPhotons_%s%s.root",ptDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());
                    else if(ptDepCorrThr!=0 && (ptBins_unfolding[i]>=ptDepCorrThr)) temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pp_pt%dto200_bkgPhotons_%s%s.root",ptDepCorrThr,cap_purHere.Data(),cap_Nfunc.Data());
                    //else temp_SBcorr=Form("SigmaIEtaIEta_Iso_nonIso_190703_temp_v31_pp_pt%dto%d_bkgPhotons_%s%s.root",(int)ptBins_unfolding[i],(int)ptBins_unfolding[i+1],cap_purHere.Data(),cap_Nfunc.Data()); 
                }
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return yj_quickPhotonPurity_v4_centDep(argv[1], argv[2]);
    if (argc == 4)
        return yj_quickPhotonPurity_v4_centDep(argv[1], argv[2], atoi(argv[3]));
    if (argc == 5)
        return yj_quickPhotonPurity_v4_centDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    if (argc == 6)
        return yj_quickPhotonPurity_v4_centDep(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    return 1;
}
