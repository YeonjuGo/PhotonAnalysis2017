...
const Int_t CENTBINS[] = {0, 200}; // for PbPb 0-100 % 
//const Int_t CENTBINS[] = {-1, 0}; // for pp data & MC
//const Int_t CENTBINS[] = {0, 60, 200};
const Int_t nCENTBINS = sizeof(CENTBINS)/sizeof(Int_t) -1;

//const Double_t PTBINS[] = {80, 9999};
...
int yj_quickPhotonPurity(const TString configFile, const TString inputData, const TString inputMC, const TString outputName) {
  ...
  //std::string triggerMC_forPurity = "(HLT_HISinglePhoton40_Eta1p5ForPPRef_v1)";//for pp MC
  std::string triggerMC_forPurity = "(HLT_HISinglePhoton40_Eta1p5_v2)";//for PbPb MC
  ...
  for (Int_t i = 0; i < nPTBINS; ++i) {
    for (Int_t j = 0; j < nCENTBINS; ++j) {
      for (Int_t k = 0; k < 1; ++k) {
        ...
        const TCut sampleIsolation = "(pho_sumIsoCorrected) < 1.0 && phoHoverE<0.1"; //for PbPb
        //const TCut sampleIsolation = "((pfcIso4<=1.37) && (pfnIso4<=1.06+0.014*phoEt+0.000019*phoEt*phoEt) && pfpIso4<=(0.28+0.0053*phoEt)) && phoHoverE<0.05"; //for pp
