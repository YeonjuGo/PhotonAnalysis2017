...
void photonRaaPlot_withJetphox_v4(TString ver="190703_temp_v31", bool doJETPHOX = true) {
    ...
    for (int i=0; i<n_hist_types; ++i) {
        ...
        for (int k=0; k<nCentBinIF; ++k) { //k=0 is 0-100 % 
            ...
            else if(hist_types[i] == "dNdpt_corr2_pp") htemp[i]->SetTitle(";E_{T}^{#gamma} (GeV/c);#frac{d^{2}#sigma^{pp}}{dp_{T}d#eta} (#frac{pb}{GeV/c})");
            else htemp[i]->SetTitle(";E_{T}^{#gamma} (GeV/c);#frac{1}{N_{evt}}#frac{1}{<T_{AA}>}#frac{d^{2}N^{PbPb}}{dp_{T}d#eta} (#frac{pb}{GeV/c})");
    ...
    for (int i=0; i<n_hist_types; ++i) {
        ...
        //for (int k=0; k<nCentBinIF; ++k) {} //k=0 is 0-100 %
            ...
            if(k==0){
                ...
                htemp[i]->SetTitle(";E_{T}^{#gamma} (GeV/c);DATA / JETPHOX");
    ...
    TGraphAsymmErrors* gr_sys_jetphox[n_hist_types][nCentBinIF];
    //pbpb
    for (int i=0; i<n_hist_types; ++i) {
        for (int k=0; k<1; ++k) { //k=0 is 0-100 %
            //for (int k=0; k<nCentBinIF; ++k) {} //k=0 is 0-100 %
            ...
            gr_sys_jetphox[i][k] = new TGraphAsymmErrors(nPtBin-rejectPtBins[k]);
            gr_sys_jetphox[i][k]->SetName(Form("gr_sys_jetphox_%d_%d",i,k));
            gr_sys_jetphox[i][k]->SetFillStyle(3005);
            gr_sys_jetphox[i][k]->SetFillColor(kAzure-1); 
            gr_sys_jetphox[i][k]->SetLineColor(kAzure-1); 
            //gr_sys_jetphox[i][k]->SetFillStyle(3013);
            //gr_sys_jetphox[i][k]->SetFillColor(12); 
            ...
            for (int ipt=1; ipt<=nPtBin-rejectPtBins[k]; ++ipt) {
                ...
                gr_sys_jetphox[i][k]->SetPointError(ipt-1, (bin_width/2), (bin_width/2), error, error);
                gr_sys_jetphox[i][k]->SetPoint(ipt-1, x, 1);
            }
            if(k==0) gr_sys_jetphox[i][k]->Draw("2");
        }
    }

    cout << "JETPHOX legend" << endl;
    ////////////////////////////////////////////////////////////////////////////////////////
    // Draw jetphox legend in each ratio and raa plots for PbPb
    ...
    for (int i=0; i<n_hist_types; ++i) {
       ...
       // if(i==0) leg_jp[i]->AddEntry(hjp_draw_raa[0],"JETPHOX PbPb(EPPS16+CT14)/pp(CT14)","l");
        leg_jp[i]->AddEntry(gr_sys_jetphox[i][0],"JETPHOX PDF Uncertainty","f");
    ...
    leg_jp[i]->AddEntry(gr_raa_sys_pdf[0],"JETPHOX Scale Uncertainty (#font[12]{E_{T}^{#gamma}/2 < #mu < 2E_{T}^{#gamma}})","f");
    c_sep[0][0]->cd();
    leg_jp[i]->Draw("same");
    ...
    c[1]->cd(1);
    TLegend* leg_dndpt = new TLegend(0.60,0.50,0.94,0.85);
    legStyle(leg_dndpt);
    ...
    c[2]->cd(1);
    TLegend* leg_dndpt_pp = new TLegend(0.60,0.70,0.94,0.85);
    legStyle(leg_dndpt_pp);
    leg_dndpt_pp->AddEntry(h1D_nominal[2][0],Form("pp%s",""),"plf");
    leg_dndpt_pp->AddEntry(hjp_draw_dndpt[1][0],"JETPHOX (CT14 NLO)","l");
    leg_dndpt_pp->Draw("same");
    double legBottom_pp = leg_dndpt_pp->GetY1();
    drawText("|#eta^{#gamma}| < 1.44",xpos_r-0.19,legBottom_pp-1*dy,1,kBlack,0.04); 
...
TGraphAsymmErrors* hist_to_graph(TGraphAsymmErrors* gr=0, TH1D* h1=0, TH1D* h2=0, TH1D* h3=0){
    int np = gr->GetN();
    TGraphAsymmErrors* new_gr = (TGraphAsymmErrors*) gr->Clone(Form("%s_dividedBy_%s",gr->GetName(),h1->GetName()));
    //TGraphAsymmErrors* new_gr = new TGraphAsymmErrors(np);
    new_gr->SetName(Form("%s_dividedBy_%s",gr->GetName(),h1->GetName()));
    for (int p=0; p<np; ++p) {                                         
        ...
        double sysUp = h2->GetBinContent(p+1);        
        double sysDown = h3->GetBinContent(p+1);        
        double scaleF = h1->GetBinContent(p+1);        
        gr->GetPoint(p,Xmean,Ymean); 
        new_gr->SetPoint(p,Xmean,Ymean/scaleF); 
        double Xerr_l= gr->GetErrorXlow(p); 
