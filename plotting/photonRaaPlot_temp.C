...
void photonRaaPlot_temp(TString ver="180805_temp_v21", bool doJETPHOX = true) {
    ...
    const std::string input_file_nominal = Form("%sresults/output/Unfold_%s_nominal.root",dir.Data(),ver.Data());
    const std::string input_file = Form("%sresults/output/phoRaa_%s_nominal.root",dir.Data(),ver.Data());
    const std::string sys_file = Form("%ssystematics/output/systematics_180817_temp_v23_cent0to100.root",dir.Data());
    ...
    TFile* input_nominal = new TFile(input_file_nominal.c_str(), "read");
