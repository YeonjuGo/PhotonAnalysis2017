...
//void test_drawRaaXsec_v3(TString ver="180731_temp_v19_nominal", bool doSplitPD = true,  bool purityTest = false)
void test_drawRaaXsec_v3(TString ver="180802_temp_v20_nominal", bool doSplitPD = true,  bool purityTest = false)
{
    TH1::SetDefaultSumw2();
    ...
    TFile* f_raw = new TFile(Form("/home/goyeonju/CMS/2017/PhotonAnalysis2017/results/output/rawDist_test_4%s.root",cap.Data()));
    ...
    else c1->SaveAs(Form("%sfigures/phoRaa_test_4_%s.pdf",dir.Data(),ver.Data()));
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(j==0) h1D_corr[2][j]->Draw("pe");
        else h1D_corr[2][j]->Draw("pe same");
        l2->AddEntry(h1D_corr[2][j],Form("%d%s-%d%s",centBins_i[j]/2,"%",centBins_f[j]/2,"%")); 
    }
    h1D_corrpp[2]->Draw("pe same");
    l2->Draw("same");
    if(purityTest) c2->SaveAs(Form("%sfigures/phoXsec_%s_purityTest.pdf",dir.Data(),ver.Data()));
    else c2->SaveAs(Form("%sfigures/phoXsec_test_4_%s.pdf",dir.Data(),ver.Data()));
...
//   }
    ...
    TString outName = Form("%soutput/phoRaa_test_4_%s.root",dir.Data(),ver.Data());
