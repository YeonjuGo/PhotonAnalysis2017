...
void getRawDist_test(TString ver="180805_temp_v21_noEnergyCorr", bool doPreScale=true, bool doSplitPD=true)
{
    ...
    if(doSplitPD){
        if(doPreScale){
            ...
            tpp_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[1]->GetName()), Form("HLT_HISinglePhoton40_Eta1p5_v1_Prescl * L1_SingleEG20_BptxAND_Prescl*(%s && %s && %s && %s)", trigCut_pp_high.GetTitle(), phoSignalCut_woTrig_pp.GetTitle(), etaCut.GetTitle(), ptSplitCut[1].GetTitle())); 
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(doSplitPD){
            if(doPreScale){
                ...
                tpbpb_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp[j][1]->GetName()), Form("HLT_HISinglePhoton40_Eta1p5_v1_Prescl * L1_SingleEG21_BptxAND*(%s && %s && %s && %s && %s)", trigCut_high.GetTitle(), phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[1].GetTitle()));
