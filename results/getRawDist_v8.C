...
// 2019July04) Not using 15-20 GeV bin 
...
void getRawDist_v8(TString ver="test", bool doPreScale=true, bool doSplitPD=true)
{
    ...
    if(doSplitPD){
        for(int i=1;i<nSplit;i++) h1D_rawpp->Add(h1D_rawpp_temp[i]); 
    }
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(doSplitPD){
            ...
            if(doPreScale){
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp_early[j][0]->GetName()), Form("HLT_HISinglePhoton20_Eta1p5_v1_Prescl * L1_MinimumBiasHF1_AND_Prescl*(L1_MinimumBiasHF1_AND_Prescl<99 && L1_MinimumBiasHF1_AND_Prescl>0 && HLT_HISinglePhoton20_Eta1p5_v1 && (run >=262620 && run <=263035) && %s && %s && %s)", phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle()));
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp_late[j][0]->GetName()), Form("HLT_HISinglePhoton20_Eta1p5_v2_Prescl * L1_MinimumBiasHF2_AND_Prescl*(L1_MinimumBiasHF2_AND_Prescl<99 && L1_MinimumBiasHF2_AND_Prescl>0 && HLT_HISinglePhoton20_Eta1p5_v2 &&  (run >=263233 && run <=263614) && %s && %s && %s)", phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle()));
            }else {
