// getRawDist.C
// Author: Yeonju Go 
// Modification : using 30 GeV trigger path(HLT_HISinglePhoton30_Eta1p5_v1) for 30-40 GeV pt bin. 
...
void getRawDist_v5_test(TString ver="190222_temp_v27_test", bool doPreScale=true, bool doSplitPD=true)
{
    ...
    TH1D* h1D_rawpp;
    TH1D* h1D_raw_temp[nCentBin+1][4];//0: low PD, 1: high PD
    TH1D* h1D_raw_temp_early[nCentBin+1][3]; //only for pbpb low PD // 3 ptbins : 15-20,20-30,30-40 
    TH1D* h1D_raw_temp_late[nCentBin+1][3];//only for pbpb low PD // 3 ptbins : 15-20,20-30,30-40
    TH1D* h1D_rawpp_temp[4];//0: low PD, 1: high PD
    ...
    ptSplitCut[1]  = Form("%s>=20 && %s<30",phoEtVar.Data(),phoEtVar.Data()); 
    ptSplitCut[2]  = Form("%s>=30 && %s<40",phoEtVar.Data(),phoEtVar.Data()); 
    ptSplitCut[3] = Form("%s>=40",phoEtVar.Data());
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(j==0) h1D_rawpp = (TH1D*) hdummy->Clone(Form("h1D_raw_pp_temp"));
        for(int k=0;k<3;++k){
        ...
        if(doSplitPD){
            for(int i=0;i<nSplit;i++){
                h1D_raw_temp[j][i] = (TH1D*) hdummy->Clone(Form("h1D_raw_cent%d_split%d",j,i)); //early+late
    ...
    if(doSplitPD){
        if(doPreScale){
            ...
            tpp->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[1]->GetName()), Form("HLT_HISinglePhoton20_Eta1p5_v1_Prescl * L1_SingleEG5_BptxAND_Prescl*(%s && %s && %s && (%s || %s))",trigCut_pp_low.GetTitle(), phoSignalCut_woTrig_pp.GetTitle(), etaCut.GetTitle(), ptSplitCut[1].GetTitle(), ptSplitCut[2].GetTitle())); 
            cout << "pp 20-40 GeV done"<<endl;
            tpp_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[2]->GetName()), Form("(%s && %s && %s && %s)", trigCut_pp_high.GetTitle(), phoSignalCut_woTrig_pp.GetTitle(), etaCut.GetTitle(), ptSplitCut[3].GetTitle())); 
            ...
            tpp_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[3]->GetName()), trigCut_pp_high && phoSignalCut_woTrig_pp && etaCut && ptSplitCut[3]); 
        }
    } else {
        if(doPreScale){
    ...
    if(doSplitPD){
        for(int i=0;i<nSplit-1;i++) h1D_rawpp->Add(h1D_rawpp_temp[i]); 
    }
    ...
    TH1D* h1D_rawpp_store = (TH1D*) h1D_rawpp->Clone("h1D_raw_pp");
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(doSplitPD){
            if(doPreScale){
                ...
                cout << "pbpb early 20-30 GeV done" << endl;
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp_early[j][2]->GetName()), Form("HLT_HISinglePhoton30_Eta1p5_v1_Prescl*L1_SingleEG7_BptxAND_Prescl*(L1_SingleEG7_BptxAND_Prescl<99 && L1_SingleEG7_BptxAND_Prescl>0 && HLT_HISinglePhoton30_Eta1p5_v1 && (run >=262620 && run <=263035) && %s && %s && %s && %s)", phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[2].GetTitle()));
                cout << "pbpb early 30-40 GeV done" << endl;
                ...
                cout << "pbpb late 20-30 GeV done" << endl;
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp_late[j][2]->GetName()), Form("HLT_HISinglePhoton30_Eta1p5_v1_Prescl*L1_SingleEG7_BptxAND_Prescl*(L1_SingleEG7_BptxAND_Prescl<99 && L1_SingleEG7_BptxAND_Prescl>0 && HLT_HISinglePhoton30_Eta1p5_v1 && (run >= 263233 && run <= 263614) && %s && %s && %s && %s)", phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[2].GetTitle()));
                cout << "pbpb late 30-40 GeV done" << endl;
                tpbpb_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp[j][1]->GetName()), Form("(%s && %s && %s && %s && %s)", trigCut_high.GetTitle(), phoSignalCut_woTrig.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[3].GetTitle()));
    ...
    if(doPreScale){
        for(int j=0;j<nCentBinIF;j++){
            if(doSplitPD){
                ...
                h1D_raw_temp[j][0]->Add(h1D_raw_temp_early[j][1]);//low PD, early run, 20 to 30GeV
                ...
                h1D_raw_temp[j][0]->Add(h1D_raw_temp_late[j][1]);//low PD, late run, 20 to 30GeV
