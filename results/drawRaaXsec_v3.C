...
void drawRaaXsec_v3(TString ver="180731_temp_v19_nominal", bool doSplitPD = true,  bool purityTest = false)
{
    TH1::SetDefaultSumw2();
    ...
    for(int j=1;j<nCentBin+1;j++)
    {
        ...
        if(i==1 || i==3) drawText(Form("%d%s-%d%s",centBins_i[j]/2,"%",centBins_f[j]/2,"%"),0.3,0.8);
