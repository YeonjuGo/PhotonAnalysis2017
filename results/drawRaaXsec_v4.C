...
void drawRaaXsec_v4(TString ver="180731_temp_v19_nominal", bool doSplitPD = true,  bool purityTest = false)
{
    TH1::SetDefaultSumw2();
    ...
    for(int j=0;j<nCentBinIF;++j)
    {
        ...
        cout << "cent "<< j << endl; 
        h1D_raw_tp[j] = (TH1D*) f_raw->Get(Form("roounfold_res_%d_rd",j));
        h1D_raw[j] = (TH1D*) h1D_raw_tp[j]->Clone(Form("h1D_raw_cent%d",j)); 
        //h1D_raw_tp[j] = (TH1D*) f_raw->Get(Form("h1D_raw_cent%d",j));
        //h1D_raw[j] = removeUnderOverBin(h1D_raw_tp[j]);
       //  ctemp[j]->cd(1);
       // h1D_raw[j]->Draw();
