...
void getRawDist(TString ver="180628_temp_v18_noEnergyCorr", bool doPreScale=false, bool doSplitPD=true)
{
    ...
    if(doSplitPD) cout << "PP skim file for high pt photons : " << ppDatafname_high << endl;
    if(doSplitPD) cout << "PbPb skim file for high pt photons : " << pbpbDatafname_high << endl;
    ...
    for(int j=0;j<nCentBinIF;j++){
        ...
        if(doSplitPD){
            if(doPreScale){
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp[j][0]->GetName()), Form("phoPreScale*(%s && %s && %s && %s)", phoSignalCut.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[0].GetTitle()));
                tpbpb_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp[j][1]->GetName()), Form("phoPreScale*(%s && %s && %s && %s)", phoSignalCut.GetTitle(), centCut.GetTitle(), etaCut.GetTitle(), ptSplitCut[1].GetTitle()));
            } else{
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw_temp[j][0]->GetName()), phoSignalCut && centCut && etaCut && ptSplitCut[0]);
                tpbpb_high->Draw(Form("%s>>+%s",phoEtVar.Data(),h1D_raw_temp[j][1]->GetName()), phoSignalCut && centCut && etaCut && ptSplitCut[1]);
            }
        } else{
            if(doPreScale)
                tpbpb->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_raw[j]->GetName()), Form("phoPreScale*(%s && %s && %s)", phoSignalCut.GetTitle(), centCut.GetTitle(), etaCut.GetTitle()));
            else 
    ...
    if(doSplitPD){
        if(doPreScale){
            tpp->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[0]->GetName()), Form("phoPreScale*(%s && %s && %s)", phoSignalCut_pp.GetTitle(), etaCut.GetTitle(), ptSplitCut[0].GetTitle())); 
            tpp_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[1]->GetName()), Form("phoPreScale*(%s && %s && %s)", phoSignalCut_pp.GetTitle(), etaCut.GetTitle(), ptSplitCut[1].GetTitle())); 
        } else{
            tpp->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[0]->GetName()), phoSignalCut_pp && etaCut && ptSplitCut[0]); 
            tpp_high->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp_temp[1]->GetName()), phoSignalCut_pp && etaCut && ptSplitCut[1]); 
        }
    } else {
        if(doPreScale)
            tpp->Draw(Form("%s>>%s",phoEtVar.Data(),h1D_rawpp->GetName()), Form("phoPreScale*(%s && %s)", phoSignalCut_pp.GetTitle(), etaCut.GetTitle())); 
        else
    ...
    TFile* outf = new TFile(Form("%soutput/test_rawDist_%s%s.root",dir.Data(),ver.Data(),cap.Data()),"recreate");
    outf->cd();
    for(int j=0;j<nCentBinIF;j++){
        h1D_raw[j]->Write();
        if(doSplitPD){
