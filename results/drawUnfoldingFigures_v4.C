...
//void drawUnfoldingFigures_v4(TString ver="190703_temp_v31_sys_unfolding_MCsize", bool doSplitMC = true)
void drawUnfoldingFigures_v4(TString ver="190703_temp_v31_nominal", bool doSplitMC = true)
{
    ...
    TLegend* l11 = new TLegend(0.4742479,0.6763028,0.9351224,0.8916344);
    legStyle(l11);
    l11->AddEntry(h1D_pbpb[0][0][0],"Measured DATA", "l");
    l11->AddEntry(h1D_pbpb[0][2][0],"Backfolded DATA", "pl");
        
    TLegend* l12 = new TLegend(0.2742479,0.7763028,0.9351224,0.8916344);
    legStyle(l12);
    l12->AddEntry(ratio[0][2][0],"Backfolded DATA / Measured DATA", "l");
        
    TLegend* l21 = new TLegend(0.4742479,0.6763028,0.9351224,0.8916344);
    legStyle(l21);
    l21->AddEntry(h1D_pbpb[1][1][0],"Unfolded Reco MC", "pl");
    l21->AddEntry(h1D_pbpb[3][0][0],"Gen MC", "pl");
        
    TLegend* l22 = new TLegend(0.2742479,0.7763028,0.9351224,0.8916344);
    legStyle(l22);
    l22->AddEntry(ratio_extra[0][0],"Unfolded Reco MC / Gen MC", "pl");
        
    TLegend* l31 = new TLegend(0.4742479,0.6763028,0.9351224,0.8916344);
    legStyle(l31);
    l31->AddEntry(h1D_pbpb[3][2][0],"Folded Gen MC", "pl");
    l31->AddEntry(h1D_pbpb[1][0][0],"Reco MC", "pl");
        
    TLegend* l32 = new TLegend(0.2742479,0.7763028,0.9351224,0.8916344);
    legStyle(l32);
    l32->AddEntry(ratio_extra[1][0],"Folded Gen MC / Reco MC", "pl");
   
    TCanvas* csep[nCentBinIF][3]; 
    ...
    for(int j=0;j<nCentBinIF;++j){
        ...
        ctot[j]->cd(1); // DATA
        gPad->SetLogy();
        SetHistTextSize(htemp_data);
        htemp_data->GetXaxis()->SetRangeUser(ptBins_unfolding_centDep[0], ptBins_unfolding_centDep[nPtBin_unfolding_centDep]);
        htemp_data->DrawCopy();
        for(int k=0;k<3;++k){
            SetHistTextSize(h1D_pbpb[0][k][j]);
    ...
    cout << "sssaasg " << endl;
        ...
        for(int k=0;k<3;++k){
            csep[j][k] = new TCanvas(Form("csep%d_%d",j,k),"",400,600);
            ratioPanelCanvas(csep[j][k]);
        }
        csep[j][0]->cd(1);
        gPad->SetLogy(); 
        htemp_data->GetYaxis()->SetRangeUser(0.001,10000);
        htemp_data->DrawCopy();
        h1D_pbpb[0][0][j]->Draw("same");//data, measured
        h1D_pbpb[0][2][j]->Draw("same");//data, backfolded
        l11->Draw("same");
        drawText(Form("%d%s-%d%s",centBins_i[j]/2,"%",centBins_f[j]/2,"%"),0.2,0.2);
        csep[j][0]->cd(2);
        SetHistTextSize(ratio[0][2][j]);
        ratio[0][2][j]->GetYaxis()->SetRangeUser(0.8,1.2);
        ratio[0][2][j]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio[0][2][j]->Draw();
        l12->Draw("same");
    ...
    cout << "aasg " << endl;
        ...
        gPad->SetLogy(); 
        htemp_data->DrawCopy();
        h1D_pbpb[1][1][j]->Draw("same");//reco_genMatched mc, unfolded 
        h1D_pbpb[3][0][j]->Draw("same"); //gen dist
        drawText(Form("%d%s-%d%s",centBins_i[j]/2,"%",centBins_f[j]/2,"%"),0.2,0.2);
        l21->Draw("same");
        csep[j][1]->cd(2);
        SetHistTextSize(ratio_extra[0][j]);
        ratio_extra[0][j]->Divide(h1D_pbpb[1][1][j],h1D_pbpb[3][0][j]);// reco_unfolded/gen
        ratio_extra[0][j]->SetMarkerStyle(markerHere[0]);
        ratio_extra[0][j]->GetYaxis()->SetRangeUser(0.8,1.2);
        ratio_extra[0][j]->DrawCopy("same");
        ratio_extra[0][j]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio_extra[0][j]->Draw();
        l22->Draw("same");
        jumSun(ptBins_unfolding_centDep[0],1,ptBins_unfolding_centDep[nPtBin_unfolding_centDep],1);
    ...
    cout << "kksssaasg " << endl;
        ...
        gPad->SetLogy(); 
        htemp_data->DrawCopy();
        h1D_pbpb[3][0][j]->Draw("same");//reco_genMatched mc, unfolded 
        h1D_pbpb[3][2][j]->Draw("same"); //gen dist
        drawText(Form("%d%s-%d%s",centBins_i[j]/2,"%",centBins_f[j]/2,"%"),0.2,0.2);
        l31->Draw("same");
        csep[j][2]->cd(2);
        SetHistTextSize(ratio_extra[1][j]);
        ratio_extra[1][j]->Divide(h1D_pbpb[3][2][j],h1D_pbpb[1][0][j]);// folded_gen/reco
        ratio_extra[1][j]->SetMarkerStyle(markerHere[0]);
        ratio_extra[1][j]->GetYaxis()->SetRangeUser(0.8,1.2);
        if(j==3) ratio_extra[1][j]->GetXaxis()->SetRangeUser(20,1.2);
        ratio_extra[1][j]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio_extra[1][j]->DrawCopy("same");
        ratio_extra[1][j]->Draw();
        l32->Draw("same");
        jumSun(ptBins_unfolding_centDep[0],1,ptBins_unfolding_centDep[nPtBin_unfolding_centDep],1);
    ...
    cout << "kksssaasg " << endl;
        ...
        for(int k=0;k<3;++k){
            csep[j][k]->SaveAs(Form("%sfigures/unfoldingFigures_%s_cent%dto%d%s_closure%d.pdf",dir.Data(),ver.Data(),centBins_i[j]/2,centBins_f[j]/2,cap.Data(),k));
    ...
    TCanvas* csep_pp[3];
    for(int k=0;k<3;++k){
        csep_pp[k] = new TCanvas(Form("csep_pp_%d",k),"",400,600);
        ratioPanelCanvas(csep_pp[k]); 
    }
        ...
        htemp_data->GetYaxis()->SetRangeUser(0.001,10000);
        htemp_data->DrawCopy();
        h1D_pp[0][0]->Draw("same");//data, measured
        h1D_pp[0][2]->Draw("same");//data, backfolded
        l11->Draw("same");
        drawText("pp",0.2,0.2);
        csep_pp[0]->cd(2);
        SetHistTextSize(ratio_pp[0][2]);
        ratio_pp[0][2]->GetYaxis()->SetRangeUser(0.8,1.2);
        ratio_pp[0][2]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio_pp[0][2]->DrawCopy();
        jumSun(ptBins_unfolding[0],1,ptBins_unfolding[nPtBin_unfolding],1);
        l12->Draw("same");
        ...
        gPad->SetLogy(); 
        htemp_data->DrawCopy();
        h1D_pp[1][1]->Draw("same");//reco_genMatched mc, unfolded 
        h1D_pp[3][0]->Draw("same"); //gen dist
        drawText("pp",0.2,0.2);
        l21->Draw("same");
        csep_pp[1]->cd(2);
        SetHistTextSize(ratio_extra[0][0]);
        ratio_extra[0][0]->Divide(h1D_pp[1][1],h1D_pp[3][0]);// reco_unfolded/gen
        ratio_extra[0][0]->SetMarkerStyle(markerHere[0]);
        ratio_extra[0][0]->GetYaxis()->SetRangeUser(0.8,1.2);
        ratio_extra[0][0]->DrawCopy("same");
        ratio_extra[0][0]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio_extra[0][0]->DrawCopy();
        jumSun(ptBins_unfolding[0],1,ptBins_unfolding[nPtBin_unfolding],1);
        l22->Draw("same");
        ...
        gPad->SetLogy(); 
        htemp_data->DrawCopy();
        h1D_pp[3][0]->Draw("same");//reco_genMatched mc, unfolded 
        h1D_pp[3][2]->Draw("same"); //gen dist
        drawText("pp",0.2,0.2);
        l31->Draw("same");
        csep_pp[2]->cd(2);
        SetHistTextSize(ratio_extra[1][0]);
        ratio_extra[1][0]->Divide(h1D_pp[3][2],h1D_pp[1][0]);// folded_gen/reco
        ratio_extra[1][0]->SetMarkerStyle(markerHere[0]);
        ratio_extra[1][0]->GetYaxis()->SetRangeUser(0.8,1.2);
        ratio_extra[1][0]->SetTitle(";E_{T}^{#gamma} (GeV);Ratio");
        ratio_extra[1][0]->DrawCopy("same");
        ratio_extra[1][0]->DrawCopy();
        jumSun(ptBins_unfolding[0],1,ptBins_unfolding[nPtBin_unfolding],1);
        l32->Draw("same");


    for(int k=0;k<3;++k){
            csep_pp[k]->SaveAs(Form("%sfigures/unfoldingFigures_%s_pp%s_closure%d.pdf",dir.Data(),ver.Data(),cap.Data(),k));
    }
