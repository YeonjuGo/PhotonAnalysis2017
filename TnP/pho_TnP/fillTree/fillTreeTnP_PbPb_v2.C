...
void fillTreeTnP_PbPb_v2() {
   ...
   //move it for mc as well, when forest is ready
...
#ifdef MC
   cout << "include when forest is ready" << endl;
#else
...
#ifdef MC
   cout << "include hltobj chain where forest is ready" << endl;
#else
...
#ifdef MC
   TFile *fout = new TFile("output_mc_PbPb_v2.root","RECREATE");
#else
   TFile *fout = new TFile("output_data_PbPb_v2.root","RECREATE");
#endif
