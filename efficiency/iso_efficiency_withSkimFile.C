...
void iso_efficiency_withSkimFile(TString coll="pp", TString ver="test_Island_sigIEtaIEta", bool doSeparation=true){
    ...
    Int_t nEff = 4;//0(tot):noiseCut && hoeCut && sigmaCut && isoCut, 1(sigma):noiseCut && hoeCut && sigmaCut, 2(hoe):noiseCut && hoeCut, 3(noise):noise//
    Int_t nEffloop = nEff;
    if(doSeparation==0) nEffloop = 1;
    TString effSt[] = {"tot","sigma","hoe","noise"};
    TString effSt_legend[] = {"total(noise & H/E & #sigma_{i#etai#eta} & sumIso)","noise & H/E & #sigma_{i#etai#eta}","noise & H/E","noise"};
    ...
    TCut numCut[nEff];
    numCut[0] = noiseCut && hoeCut && sigmaCut && isoCut;
    numCut[1] = noiseCut && hoeCut && sigmaCut;
    numCut[2] = noiseCut && hoeCut;
    numCut[3] = noiseCut;
    ...
    if(coll=="pp") {
        ...
        numCut[0] = noiseCut && hoeCut_pp && sigmaCut_pp && isoCut_pp; 
        numCut[1] = noiseCut && hoeCut_pp && sigmaCut_pp; 
        numCut[2] = noiseCut && hoeCut_pp; 
        numCut[3] = noiseCut; 
    }
    for(Int_t i=0;i<nCENTBINS;++i){
    ...
    c1->SaveAs(Form("%sfigures/%s_tot_iso_efficiency_centDep_%s.pdf",dir.Data(),coll.Data(),ver.Data()));
