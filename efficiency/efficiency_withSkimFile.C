...
void efficiency_withSkimFile(TString coll="pbpb", TString ver="170523_temp_v1"){
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        t1->Draw(Form("phoEtCorrected>>%s",sig_den[i]->GetName()),mcIsolation && commonCut[i]);
        sig_den[i]=(TH1D*)gDirectory->Get(sig_den[i]->GetName());
        t1->Draw(Form("phoEtCorrected>>%s",sig_num[i]->GetName()),mcIsolation && commonCut[i] && numCut); 
        sig_num[i]=(TH1D*)gDirectory->Get(sig_num[i]->GetName());
        t2->Draw(Form("phoEtCorrected>>%s",bkg_den[i]->GetName()),mcBkgIsolation && commonCut[i]);
        bkg_den[i]=(TH1D*)gDirectory->Get(bkg_den[i]->GetName());
        t2->Draw(Form("phoEtCorrected>>%s",bkg_num[i]->GetName()),mcBkgIsolation && commonCut[i] && numCut); 
