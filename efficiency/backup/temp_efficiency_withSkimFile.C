...
void temp_efficiency_withSkimFile(TString coll="pbpb", TString ver="170523_temp_v1"){
    TH1::SetDefaultSumw2();
    gStyle->SetOptStat(0);
    SetyjPadStyle();
    ...
    TGraphAsymmErrors* g_sig_eff[nCENTBINS];
    TGraphAsymmErrors* g_bkg_eff[nCENTBINS];
    ...
    TCut numCut = NonIsoSBCut; 
    //TCut numCut = evtSelFilterCut && spikeRejection && hotspotCut && electronCut && sigmaCut && hoeCut && NonIsoSBCut; 
    for(Int_t i=0;i<nCENTBINS;++i){
        centCut[i] = Form("(hiBin>=%d)&&(hiBin<%d)",centBins_i[i],centBins_f[i]);
        commonCut[i] = trigCut_mc_pbpb && etaCut && centCut[i] && evtSelFilterCut && spikeRejection && hotspotCut && electronCut && sigmaCut && hoeCut; 
        //commonCut[i] = trigCut_mc_pbpb && etaCut && centCut[i];
    } 

    if(coll=="pp") {
        commonCut[0] = trigCut_mc_pp && etaCut && evtSelFilterCut_pp && spikeRejection && hotspotCut && electronCut && sigmaCut && hoeCut; 
        numCut = NonIsoSBCut; 
    }
    for(Int_t i=0;i<nCENTBINS;++i){
        t1->Draw(Form("phoEt>>%s",sig_den[i]->GetName()),mcIsolation && commonCut[i]);
        sig_den[i]=(TH1D*)gDirectory->Get(sig_den[i]->GetName());
        t1->Draw(Form("phoEt>>%s",sig_num[i]->GetName()),mcIsolation && commonCut[i] && numCut); 
        sig_num[i]=(TH1D*)gDirectory->Get(sig_num[i]->GetName());
        t2->Draw(Form("phoEt>>%s",bkg_den[i]->GetName()),mcBkgIsolation && commonCut[i]);
        bkg_den[i]=(TH1D*)gDirectory->Get(bkg_den[i]->GetName());
        t2->Draw(Form("phoEt>>%s",bkg_num[i]->GetName()),mcBkgIsolation && commonCut[i] && numCut); 
        bkg_num[i]=(TH1D*)gDirectory->Get(bkg_num[i]->GetName());
    }
    for(Int_t i=0;i<nCENTBINS;++i){
        g_sig_eff[i] = new TGraphAsymmErrors();
        g_sig_eff[i]->SetName(Form("g_%s",sig_eff[i]->GetName()));
        g_sig_eff[i]->SetTitle(sig_eff[i]->GetTitle());
        g_sig_eff[i]->BayesDivide(sig_num[i],sig_den[i]);
        
        g_bkg_eff[i] = new TGraphAsymmErrors();
        g_bkg_eff[i]->SetName(Form("g_%s",bkg_eff[i]->GetName()));
        g_bkg_eff[i]->SetTitle(bkg_eff[i]->GetTitle());
        g_bkg_eff[i]->BayesDivide(bkg_num[i],bkg_den[i]);
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
       ...
       // sig_eff[i]->SetMarkerStyle(24);
       // bkg_eff[i]->SetMarkerStyle(24);
       // sig_eff[i]->SetLineColor(colHere[i]);
       // bkg_eff[i]->SetLineColor(colHere[i]);
       // sig_eff[i]->SetMarkerColor(colHere[i]);
       // bkg_eff[i]->SetMarkerColor(colHere[i]);
       // sig_eff[i]->GetYaxis()->SetRangeUser(0,1);
       // sig_eff[i]->GetXaxis()->CenterTitle();
       // sig_eff[i]->GetYaxis()->CenterTitle();
       ...
       // else sig_eff[i]->Draw("same pe");
        ...
        g_sig_eff[i]->SetMarkerStyle(28);
        g_sig_eff[i]->SetMarkerColor(colHere[i]);
        //g_sig_eff[i]->Draw("same p");
    ...
    c1->SaveAs(Form("%sfigures/temp_%s_efficiency_%s.pdf",dir.Data(),coll.Data(),ver.Data()));
    //c1->SaveAs(Form("%sfigures/%s_efficiency_%s.pdf",dir.Data(),coll.Data(),ver.Data()));

    TFile* fout = new TFile(Form("%soutput/temp_%s_efficiency_%s.root",dir.Data(),coll.Data(),ver.Data()),"RECREATE");
