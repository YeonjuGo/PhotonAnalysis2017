...
#include "../phoRaaCuts/phoRaaCuts_eff_etaDep.h"
...
void iso_efficiency_withSkimFile_etaDep(TString coll="pp", TString ver="180802_temp_v20", int pt_i=20, int pt_f=30, bool doWeight=true, bool doBkg=false, bool doSeparation=true){
    ...
    const double EtaBins[] = {-1.44,-1.20,-1.00,-0.75,-0.50,-0.25,0.00,0.25,0.50,0.75,1.00,1.20,1.44}; 
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        if(isConsBin) {
            ...
            bkg_den[i] = new TH1D(Form("bkg_den_cent%d",i),";#eta^{#gamma};dN/d#eta",nEtaBin,EtaBins);
            sig_den[i] = new TH1D(Form("sig_den_cent%d",i),";#eta^{#gamma};dN/d#eta",nEtaBin,EtaBins);
        } 
        for(Int_t j=0;j<nEffloop;++j){
            bkg_num[i][j] = (TH1D*) bkg_den[i]->Clone(Form("bkg_num_cent%d_%s",i,effSt[j].Data()));
            bkg_eff[i][j] = (TH1D*) bkg_den[i]->Clone(Form("bkg_eff_cent%d_%s",i,effSt[j].Data()));
            sig_num[i][j] = (TH1D*) sig_den[i]->Clone(Form("sig_num_cent%d_%s",i,effSt[j].Data()));
            sig_eff[i][j] = (TH1D*) sig_den[i]->Clone(Form("sig_eff_cent%d_%s",i,effSt[j].Data()));
           // if(isConsBin) {
    ...
    numCut[4] = noiseCut;
    TCut ptCut_ = Form("phoEtCorrected>=%f && phoEtCorrected<%f",(float)pt_i,(float)pt_f);
    for(Int_t i=0;i<nCENTBINS;++i){
        centCut[i] = Form("(hiBin>=%d)&&(hiBin<%d)",centBins_i[i],centBins_f[i]);
        //commonCut[i] = trigCut_mc && etaCut && centCut[i];
        commonCut[i] = trigCut_mc && evtSelFilterCut && centCut[i] && ptCut_;
    } 
    if(coll=="pp") {
        //commonCut[0] = trigCut_mc_pp && etaCut; 
        commonCut[0] = trigCut_mc_pp && evtSelFilterCut_pp && ptCut_; 
    ...
    //Fill histograms
    for(Int_t i=0;i<nCENTBINS;++i){
        if(doWeight)    t1->Draw(Form("%s>>%s","phoEta",sig_den[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcIsolation.GetTitle(),commonCut[i].GetTitle()));
        else            t1->Draw(Form("%s>>%s","phoEta",sig_den[i]->GetName()), mcIsolation && commonCut[i]);
        sig_den[i]=(TH1D*)gDirectory->Get(sig_den[i]->GetName());
        if(doBkg){ 
            if(doWeight)    t1->Draw(Form("%s>>%s","phoEta",bkg_den[i]->GetName()), Form("(%s)*((%s) && (%s))","weight",mcBkgIsolation.GetTitle(),commonCut[i].GetTitle()));
            else            t1->Draw(Form("%s>>%s","phoEta",bkg_den[i]->GetName()),mcBkgIsolation && commonCut[i]);
            bkg_den[i]=(TH1D*)gDirectory->Get(bkg_den[i]->GetName());
        }
        for(Int_t j=0;j<nEffloop;++j){
            if(doWeight)    t1->Draw(Form("%s>>%s","phoEta",sig_num[i][j]->GetName()),Form("(%s)*((%s) && (%s) && (%s))","weight",mcIsolation.GetTitle(), commonCut[i].GetTitle(), numCut[j].GetTitle())); 
            else            t1->Draw(Form("%s>>%s","phoEta",sig_num[i][j]->GetName()), mcIsolation && commonCut[i] && numCut[j]); 
            sig_num[i][j]=(TH1D*)gDirectory->Get(sig_num[i][j]->GetName());
            if(doBkg){
                if(doWeight)    t1->Draw(Form("%s>>%s","phoEta",sig_num[i][j]->GetName()),Form("(%s)*((%s) && (%s) && (%s))","weight",mcBkgIsolation.GetTitle(), commonCut[i].GetTitle(), numCut[j].GetTitle())); 
                else            t1->Draw(Form("%s>>%s","phoEta",bkg_num[i][j]->GetName()),mcBkgIsolation && commonCut[i] && numCut[j]); 
   ...
   //     }}
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        hTemp[i] = (TH1D*) sig_eff[i][0]->Clone(Form("htemp_%d",i));
        hTemp[i]->SetMarkerColor(colHere[i]);
        hTemp[i]->SetMarkerStyle(markerStyle[i]);
       ...
       // }
        ...
        sig_eff[i][0]->SetTitle(";#eta^{#gamma};Isolation Efficiency");
        if(i==0) sig_eff[i][0]->DrawCopy("p");
        else sig_eff[i][0]->DrawCopy("same p");
        if(coll=="pbpb") l1->AddEntry(hTemp[i], Form("%d%s-%d%s",centBins_i[i]/2,"%",centBins_f[i]/2,"%"));
        //if(coll=="pbpb") l1->AddEntry(sig_eff[i][0], Form("%d%s-%d%s",centBins_i[i]/2,"%",centBins_f[i]/2,"%"));
    } 
    if(coll=="pbpb") l1->Draw("same");
    drawText(Form("%i<p_{T}^{#gamma}<%i",pt_i,pt_f),0.2,0.2);
    drawText(Form("%s Isolation Efficiency",coll.Data()),0.2,1.0-c1->GetBottomMargin()+0.06,0,kBlack,16);
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        ...
        if(coll=="pbpb") c2[i]->SaveAs(Form("%sfigures/efficiency_iso_etaDep_%s_isoDep_%s_cent%d%s-%d%s_pt%dto%d.pdf",dir.Data(),coll.Data(),ver.Data(),centBins_i[i]/2,"%",centBins_f[i]/2,"%",pt_i,pt_f));
        else c2[i]->SaveAs(Form("%sfigures/efficiency_iso_etaDep_%s_isoDep_%s_pt%dto%d.pdf",dir.Data(),coll.Data(),ver.Data(),pt_i,pt_f));
    }
    //*/
    /////////////////////////////////////////////////////////////////////
    // Write output file  
