...
void draw_iso_efficiency_v2(TString coll="pbpb", TString ver="190703_temp_v31_nominal_backup2", bool doWeight=true, bool doBkg=false, bool doSeparation=true){
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        ...
        sig_eff[i][1]->SetMarkerStyle(markerStyle[i]);
        sig_eff[i][1]->GetYaxis()->SetRangeUser(0,1);
    ...
    for(Int_t i=0;i<nCENTBINS;++i){
        ...
        l2->Draw("same");
        if(coll=="pbpb") drawText(Form("%s %d%s-%d%s",coll.Data(),centBins_i[i]/2,"%",centBins_f[i]/2,"%"),0.60,0.45);
    ...
    //drawText(Form("%s Total Isolation Efficiency",coll.Data()),0.2,1.0-c1->GetBottomMargin()+0.06,0,kBlack,16);
    c3->SaveAs(Form("%sfigures/efficiency_withTrigEff_withTnP_%s_tot_centDep_%s.pdf",dir.Data(),coll.Data(),ver.Data()));
